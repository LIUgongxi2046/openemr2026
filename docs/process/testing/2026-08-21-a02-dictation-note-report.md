...
范围：FR-120–124 / A02 全局 AI医助小南·听写（顶栏/浮窗 UI、限频/转任务与动作审批仍待办）

## 结论

A02 全局 AI医助小南域新增听写首切：`dictation_note` 记录听写转录文本，状态机 `DRAFT→REVIEWED→SIGNED`。听写安全硬门：转录文本必填且创建后不可篡改（数据库触发器保护 transcript 与身份字段），签署前必须已复核（`SIGN` 仅允许从 `REVIEWED` 推进，状态/时间一致性由数据库约束保证）。身份字段不可变，状态迁移走乐观锁。事件、审计与 Outbox 同事务。
