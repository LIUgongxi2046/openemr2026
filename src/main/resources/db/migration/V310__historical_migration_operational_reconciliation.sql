...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'historical_migration_batch_source_fk'
  ) then
    alter table historical_migration_batch
...
create index if not exists historical_migration_batch_source_id_idx
  on historical_migration_batch (tenant_id, source_system_id, started_at desc);

create table if not exists historical_migration_reconciliation_item (
...
create index if not exists historical_migration_reconciliation_batch_idx
  on historical_migration_reconciliation_item
  (tenant_id, batch_id, validation_status, reconciled_at desc);

create or replace function prevent_historical_migration_reconciliation_mutation()
returns trigger language plpgsql as $$
begin
  raise exception 'historical migration reconciliation evidence is immutable';
end $$;

drop trigger if exists historical_migration_reconciliation_immutable
  on historical_migration_reconciliation_item;
