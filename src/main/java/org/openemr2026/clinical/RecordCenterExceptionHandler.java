...
@RestControllerAdvice(assignableTypes = RecordCenterController.class)
final class RecordCenterExceptionHandler {
    @ExceptionHandler(RecordCenterException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(RecordCenterException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "RECORD_CENTER_CONSTRAINT_CONFLICT", "The record-center command conflicts with current facts"));
    }
