...
@RestControllerAdvice(assignableTypes = SurgicalProcedureController.class)
final class SurgicalProcedureExceptionHandler {
    @ExceptionHandler(SurgicalProcedureException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(SurgicalProcedureException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "SURGICAL_PROCEDURE_CONSTRAINT_CONFLICT", "The surgical procedure command conflicts with current facts"));
    }
