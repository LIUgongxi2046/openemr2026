...
@RestControllerAdvice(assignableTypes = InfectionEventController.class)
final class InfectionEventExceptionHandler {
    @ExceptionHandler(InfectionEventException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(InfectionEventException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "INFECTION_EVENT_CONSTRAINT_CONFLICT", "The infection monitoring command conflicts with current facts"));
    }
