...
@RestControllerAdvice(assignableTypes = ObstetricDeliveryController.class)
final class ObstetricDeliveryExceptionHandler {
    @ExceptionHandler(ObstetricDeliveryException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ObstetricDeliveryException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OBSTETRIC_DELIVERY_CONSTRAINT_CONFLICT",
                "The obstetric delivery record conflicts with current facts"));
    }
