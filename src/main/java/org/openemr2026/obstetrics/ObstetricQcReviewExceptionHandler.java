...
@RestControllerAdvice(assignableTypes = ObstetricQcReviewController.class)
final class ObstetricQcReviewExceptionHandler {
    @ExceptionHandler(ObstetricQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ObstetricQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OBSTETRIC_QC_CONSTRAINT_CONFLICT",
                "The obstetric QC review conflicts with current facts"));
    }
