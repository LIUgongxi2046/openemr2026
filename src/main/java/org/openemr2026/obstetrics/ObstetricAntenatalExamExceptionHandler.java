...
@RestControllerAdvice(assignableTypes = ObstetricAntenatalExamController.class)
final class ObstetricAntenatalExamExceptionHandler {
    @ExceptionHandler(ObstetricAntenatalExamException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ObstetricAntenatalExamException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OBSTETRIC_ANTENATAL_CONSTRAINT_CONFLICT",
                "The obstetric antenatal exam conflicts with current facts"));
    }
