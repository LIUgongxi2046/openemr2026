...
@RestControllerAdvice(assignableTypes = ObstetricController.class)
final class ObstetricExceptionHandler {
    @ExceptionHandler(ObstetricException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ObstetricException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OBSTETRIC_RECORD_CONSTRAINT_CONFLICT", "The obstetric record command conflicts with current facts"));
    }
