...
@RestControllerAdvice(assignableTypes = ObstetricCareNoteController.class)
final class ObstetricCareNoteExceptionHandler {
    @ExceptionHandler(ObstetricCareNoteException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ObstetricCareNoteException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OBSTETRIC_CARE_NOTE_CONSTRAINT_CONFLICT",
