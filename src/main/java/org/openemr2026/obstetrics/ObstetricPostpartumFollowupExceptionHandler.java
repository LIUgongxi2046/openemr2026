...
@RestControllerAdvice(assignableTypes = ObstetricPostpartumFollowupController.class)
final class ObstetricPostpartumFollowupExceptionHandler {
    @ExceptionHandler(ObstetricPostpartumFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ObstetricPostpartumFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OBSTETRIC_POSTPARTUM_CONSTRAINT_CONFLICT",
                "The obstetric postpartum followup conflicts with current facts"));
    }
