...
@Service
final class ObstetricCareNoteService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    ObstetricCareNoteService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    ObstetricCareNoteWire create(
            ClinicalIdentity identity, String idempotencyKey, ObstetricCareNoteCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "OBSTETRIC_CARE_NOTE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into obstetric_care_note(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "OBSTETRIC_CARE_NOTE_CREATED", "ObstetricCareNoteCreated");
            completeCommand(identity, "OBSTETRIC_CARE_NOTE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<ObstetricCareNoteWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from obstetric_care_note
    ...
    private ObstetricCareNoteWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from obstetric_care_note where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new ObstetricCareNoteWire(
                ...
                .optional().orElseThrow(ObstetricCareNoteService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new ObstetricCareNoteException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new ObstetricCareNoteException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'OBSTETRIC_CARE_NOTE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'OBSTETRIC_CARE_NOTE', :aggregate, :version, :event_type, 1,
    ...
    private static ObstetricCareNoteException invalid(String message) {
        return new ObstetricCareNoteException("OBSTETRIC_CARE_NOTE_REQUEST_INVALID", 400, message);
    }

    static ObstetricCareNoteException contextDenied() {
        return new ObstetricCareNoteException("CONTEXT_NOT_PERMITTED", 403,
