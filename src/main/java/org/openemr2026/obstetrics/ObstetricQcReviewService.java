...
import org.openemr2026.contracts.ObstetricQcReviewCreateRequestWire;
import org.openemr2026.contracts.ObstetricQcReviewWire;
...
@Service
final class ObstetricQcReviewService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    ObstetricQcReviewService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    ObstetricQcReviewWire record(
            ClinicalIdentity identity, String idempotencyKey, ObstetricQcReviewCreateRequestWire request) {
        ...
        if (request.reviewConclusion() == ObstetricQcReviewCreateRequestWire.ReviewConclusionValue.FAIL
                && defectDescription == null) {
            throw new ObstetricQcReviewException(
                    "OBSTETRIC_QC_DEFECT_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "OBSTETRIC_QC_REVIEW", idempotencyKey,
                    sha256(request.patientId() + "|" + request.reviewedRecordType() + "|" + request.reviewedRecordId()));
            UUID reviewId = UUID.randomUUID();
            jdbc.sql("""
                    insert into obstetric_qc_review(
            ...
            appendEvidence(identity, request.patientId(), reviewId, 1, "OBSTETRIC_QC_REVIEWED",
                    "ObstetricQcReviewed");
            completeCommand(identity, "OBSTETRIC_QC_REVIEW", idempotencyKey, reviewId);
            return review(identity.tenantId(), reviewId);
        });
    }

    List<ObstetricQcReviewWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select review_id from obstetric_qc_review
    ...
    private ObstetricQcReviewWire review(UUID tenantId, UUID reviewId) {
        return jdbc.sql("""
                ...
                .optional().orElseThrow(ObstetricQcReviewService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new ObstetricQcReviewException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new ObstetricQcReviewException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'OBSTETRIC_QC_REVIEW', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'OBSTETRIC_QC_REVIEW', :aggregate, :version, :event_type, 1,
    ...
    private static ObstetricQcReviewException invalid(String message) {
        return new ObstetricQcReviewException("OBSTETRIC_QC_REQUEST_INVALID", 400, message);
    }

    static ObstetricQcReviewException contextDenied() {
        return new ObstetricQcReviewException(
                "CONTEXT_NOT_PERMITTED", 403, "The requested obstetric QC context is not permitted");
    }
