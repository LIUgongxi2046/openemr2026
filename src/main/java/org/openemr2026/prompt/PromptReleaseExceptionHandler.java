...
@RestControllerAdvice(assignableTypes = PromptReleaseController.class)
final class PromptReleaseExceptionHandler {
    @ExceptionHandler(PromptReleaseException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(PromptReleaseException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PROMPT_RELEASE_CONSTRAINT_CONFLICT", "The prompt release command conflicts with current facts"));
    }
