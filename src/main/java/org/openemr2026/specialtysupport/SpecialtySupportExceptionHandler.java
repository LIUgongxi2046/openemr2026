...
@RestControllerAdvice(assignableTypes = SpecialtySupportController.class)
final class SpecialtySupportExceptionHandler {

    @ExceptionHandler(SpecialtySupportException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(SpecialtySupportException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "SPECIALTY_SUPPORT_CONSTRAINT_CONFLICT", "The support assessment conflicts with current evidence"));
    }
