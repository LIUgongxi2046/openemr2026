...
@RestControllerAdvice(assignableTypes = ResearchCohortSnapshotController.class)
final class ResearchCohortSnapshotExceptionHandler {
    @ExceptionHandler(ResearchCohortSnapshotException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ResearchCohortSnapshotException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "RESEARCH_COHORT_SNAPSHOT_CONSTRAINT_CONFLICT",
                "The research cohort snapshot conflicts with current facts"));
    }
