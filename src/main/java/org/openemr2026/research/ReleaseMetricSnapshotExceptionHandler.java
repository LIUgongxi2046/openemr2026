...
@RestControllerAdvice(assignableTypes = ReleaseMetricSnapshotController.class)
final class ReleaseMetricSnapshotExceptionHandler {
    @ExceptionHandler(ReleaseMetricSnapshotException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ReleaseMetricSnapshotException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "RELEASE_METRIC_CONSTRAINT_CONFLICT",
                "The release metric snapshot conflicts with current facts"));
    }
