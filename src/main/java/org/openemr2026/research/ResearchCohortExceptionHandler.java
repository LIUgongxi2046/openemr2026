...
@RestControllerAdvice(assignableTypes = ResearchCohortController.class)
final class ResearchCohortExceptionHandler {
    @ExceptionHandler(ResearchCohortException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ResearchCohortException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "RESEARCH_COHORT_CONSTRAINT_CONFLICT", "The research cohort command conflicts with current facts"));
    }
