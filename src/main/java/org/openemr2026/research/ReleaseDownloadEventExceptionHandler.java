...
@RestControllerAdvice(assignableTypes = ReleaseDownloadEventController.class)
final class ReleaseDownloadEventExceptionHandler {
    @ExceptionHandler(ReleaseDownloadEventException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ReleaseDownloadEventException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "RELEASE_DOWNLOAD_DEDUP_CONFLICT",
                "The download event duplicates an existing valid download"));
    }
