...
@RestControllerAdvice(assignableTypes = ResearchDatasetController.class)
final class ResearchDatasetExceptionHandler {
    @ExceptionHandler(ResearchDatasetException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ResearchDatasetException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "RESEARCH_DATASET_CONSTRAINT_CONFLICT", "The research dataset command conflicts with current facts"));
    }
