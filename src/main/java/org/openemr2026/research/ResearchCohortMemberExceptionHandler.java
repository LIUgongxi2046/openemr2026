...
@RestControllerAdvice(assignableTypes = ResearchCohortMemberController.class)
final class ResearchCohortMemberExceptionHandler {
    @ExceptionHandler(ResearchCohortMemberException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ResearchCohortMemberException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "RESEARCH_COHORT_MEMBER_CONSTRAINT_CONFLICT",
                "The cohort member conflicts with current facts"));
    }
