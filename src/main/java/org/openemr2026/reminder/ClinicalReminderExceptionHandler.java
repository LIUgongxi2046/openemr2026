...
@RestControllerAdvice(assignableTypes = ClinicalReminderController.class)
final class ClinicalReminderExceptionHandler {
    @ExceptionHandler(ClinicalReminderException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ClinicalReminderException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "CLINICAL_REMINDER_CONSTRAINT_CONFLICT", "The reminder command conflicts with current facts"));
    }
