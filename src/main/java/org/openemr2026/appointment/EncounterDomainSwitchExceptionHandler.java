...
@RestControllerAdvice(assignableTypes = EncounterDomainSwitchController.class)
final class EncounterDomainSwitchExceptionHandler {
    @ExceptionHandler(EncounterDomainSwitchException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EncounterDomainSwitchException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ENCOUNTER_DOMAIN_SWITCH_CONSTRAINT_CONFLICT",
                "The encounter domain switch conflicts with current facts"));
    }
