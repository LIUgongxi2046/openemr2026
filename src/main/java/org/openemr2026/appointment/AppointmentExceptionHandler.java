...
final class AppointmentExceptionHandler {
    @ExceptionHandler(AppointmentException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(AppointmentException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "APPOINTMENT_CONSTRAINT_CONFLICT", "The appointment command conflicts with current scheduling facts"));
    }
