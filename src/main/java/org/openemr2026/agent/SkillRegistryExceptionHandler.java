...
@RestControllerAdvice(assignableTypes = SkillRegistryController.class)
final class SkillRegistryExceptionHandler {
    @ExceptionHandler(SkillRegistryException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(SkillRegistryException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "SKILL_REGISTRY_CONSTRAINT_CONFLICT", "The skill registry command conflicts with current facts"));
    }
