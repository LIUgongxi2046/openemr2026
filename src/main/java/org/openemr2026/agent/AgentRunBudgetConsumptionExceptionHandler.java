...
@RestControllerAdvice(assignableTypes = AgentRunBudgetConsumptionController.class)
final class AgentRunBudgetConsumptionExceptionHandler {
    @ExceptionHandler(AgentRunBudgetConsumptionException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(AgentRunBudgetConsumptionException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "AGENT_RUN_BUDGET_CONSUMPTION_CONFLICT",
                "The agent run budget consumption conflicts with current facts"));
    }
