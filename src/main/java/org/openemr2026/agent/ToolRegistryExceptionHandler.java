...
@RestControllerAdvice(assignableTypes = ToolRegistryController.class)
final class ToolRegistryExceptionHandler {
    @ExceptionHandler(ToolRegistryException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ToolRegistryException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "TOOL_REGISTRY_CONSTRAINT_CONFLICT", "The tool registry command conflicts with current facts"));
    }
