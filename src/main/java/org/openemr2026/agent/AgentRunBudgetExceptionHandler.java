...
@RestControllerAdvice(assignableTypes = AgentRunBudgetController.class)
final class AgentRunBudgetExceptionHandler {
    @ExceptionHandler(AgentRunBudgetException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(AgentRunBudgetException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "AGENT_RUN_BUDGET_CONSTRAINT_CONFLICT", "The agent run budget command conflicts with current facts"));
    }
