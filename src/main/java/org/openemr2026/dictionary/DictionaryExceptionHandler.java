...
final class DictionaryExceptionHandler {
    @ExceptionHandler(DictionaryException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DictionaryException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DICTIONARY_ITEM_CONSTRAINT_CONFLICT", "The dictionary command conflicts with current facts"));
    }
