...
@RestControllerAdvice(assignableTypes = BloodTransfusionController.class)
final class BloodTransfusionExceptionHandler {
    @ExceptionHandler(BloodTransfusionException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(BloodTransfusionException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "BLOOD_TRANSFUSION_CONSTRAINT_CONFLICT", "The transfusion command conflicts with current facts"));
    }
