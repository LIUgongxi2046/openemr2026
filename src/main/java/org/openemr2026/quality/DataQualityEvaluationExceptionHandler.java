...
@RestControllerAdvice(assignableTypes = DataQualityEvaluationController.class)
final class DataQualityEvaluationExceptionHandler {
    @ExceptionHandler(DataQualityEvaluationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DataQualityEvaluationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DATA_QUALITY_EVALUATION_CONSTRAINT_CONFLICT",
                "The data quality evaluation conflicts with current facts"));
    }
