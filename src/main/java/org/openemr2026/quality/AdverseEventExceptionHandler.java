...
@RestControllerAdvice(assignableTypes = AdverseEventController.class)
final class AdverseEventExceptionHandler {
    @ExceptionHandler(AdverseEventException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(AdverseEventException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ADVERSE_EVENT_CONSTRAINT_CONFLICT", "The adverse event command conflicts with current facts"));
    }
