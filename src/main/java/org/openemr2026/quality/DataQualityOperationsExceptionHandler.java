...
@RestControllerAdvice(assignableTypes = DataQualityOperationsController.class)
final class DataQualityOperationsExceptionHandler {
    @ExceptionHandler(DataQualityOperationsException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DataQualityOperationsException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DATA_QUALITY_OPERATION_CONSTRAINT_CONFLICT",
                "The data quality operation conflicts with current evidence"));
    }
