...
@RestControllerAdvice(assignableTypes = DataQualityRuleController.class)
final class DataQualityRuleExceptionHandler {
    @ExceptionHandler(DataQualityRuleException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DataQualityRuleException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DATA_QUALITY_RULE_CONSTRAINT_CONFLICT", "The data quality rule command conflicts with current facts"));
    }
