...
@RestControllerAdvice(assignableTypes = PediatricEvidenceController.class)
final class PediatricEvidenceExceptionHandler {
    @ExceptionHandler(PediatricEvidenceException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(PediatricEvidenceException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PEDIATRIC_EVIDENCE_CONSTRAINT_CONFLICT",
