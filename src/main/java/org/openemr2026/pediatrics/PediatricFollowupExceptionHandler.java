...
@RestControllerAdvice(assignableTypes = PediatricFollowupController.class)
final class PediatricFollowupExceptionHandler {
    @ExceptionHandler(PediatricFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(PediatricFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PEDIATRIC_FOLLOWUP_CONSTRAINT_CONFLICT",
                "The pediatric followup record conflicts with current facts"));
    }
