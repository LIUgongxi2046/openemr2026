...
@RestControllerAdvice(assignableTypes = PediatricTreatmentController.class)
final class PediatricTreatmentExceptionHandler {
    @ExceptionHandler(PediatricTreatmentException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(PediatricTreatmentException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PEDIATRIC_TREATMENT_CONSTRAINT_CONFLICT",
