...
import org.openemr2026.contracts.PediatricEvidenceCreateRequestWire;
import org.openemr2026.contracts.PediatricEvidenceWire;
...
@Service
final class PediatricEvidenceService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    PediatricEvidenceService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    PediatricEvidenceWire create(
            ClinicalIdentity identity, String idempotencyKey, PediatricEvidenceCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "PEDIATRIC_EVIDENCE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into pediatric_evidence_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "PEDIATRIC_EVIDENCE_CREATED", "PediatricEvidenceCreated");
            completeCommand(identity, "PEDIATRIC_EVIDENCE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<PediatricEvidenceWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from pediatric_evidence_record
    ...
    private PediatricEvidenceWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from pediatric_evidence_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new PediatricEvidenceWire(
                ...
                .optional().orElseThrow(PediatricEvidenceService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new PediatricEvidenceException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new PediatricEvidenceException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'PEDIATRIC_EVIDENCE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'PEDIATRIC_EVIDENCE', :aggregate, :version, :event_type, 1,
    ...
    private static PediatricEvidenceException invalid(String message) {
        return new PediatricEvidenceException("PEDIATRIC_EVIDENCE_REQUEST_INVALID", 400, message);
    }

    static PediatricEvidenceException contextDenied() {
        return new PediatricEvidenceException("CONTEXT_NOT_PERMITTED", 403,
