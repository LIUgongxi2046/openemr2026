...
@RestControllerAdvice(assignableTypes = PediatricController.class)
final class PediatricExceptionHandler {
    @ExceptionHandler(PediatricException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(PediatricException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PEDIATRIC_CONSTRAINT_CONFLICT", "The pediatric record command conflicts with current facts"));
    }
