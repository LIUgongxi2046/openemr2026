...
@Service
final class PediatricTreatmentService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    PediatricTreatmentService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    PediatricTreatmentWire create(
            ClinicalIdentity identity, String idempotencyKey, PediatricTreatmentCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "PEDIATRIC_TREATMENT_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into pediatric_treatment_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "PEDIATRIC_TREATMENT_CREATED", "PediatricTreatmentCreated");
            completeCommand(identity, "PEDIATRIC_TREATMENT_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<PediatricTreatmentWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from pediatric_treatment_record
    ...
    private PediatricTreatmentWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from pediatric_treatment_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new PediatricTreatmentWire(
                ...
                .optional().orElseThrow(PediatricTreatmentService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new PediatricTreatmentException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new PediatricTreatmentException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'PEDIATRIC_TREATMENT', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'PEDIATRIC_TREATMENT', :aggregate, :version, :event_type, 1,
    ...
    private static PediatricTreatmentException invalid(String message) {
        return new PediatricTreatmentException("PEDIATRIC_TREATMENT_REQUEST_INVALID", 400, message);
    }

    static PediatricTreatmentException contextDenied() {
        return new PediatricTreatmentException("CONTEXT_NOT_PERMITTED", 403,
