...
@RestControllerAdvice(assignableTypes = PediatricGrowthController.class)
final class PediatricGrowthExceptionHandler {
    @ExceptionHandler(PediatricGrowthException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(PediatricGrowthException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PEDIATRIC_GROWTH_CONSTRAINT_CONFLICT",
                "The pediatric growth record conflicts with current facts"));
    }
