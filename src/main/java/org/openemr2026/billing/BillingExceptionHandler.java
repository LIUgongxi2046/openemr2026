...
final class BillingExceptionHandler {
    @ExceptionHandler(BillingException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(BillingException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "CHARGE_CONSTRAINT_CONFLICT", "The charge command conflicts with current billing facts"));
    }
