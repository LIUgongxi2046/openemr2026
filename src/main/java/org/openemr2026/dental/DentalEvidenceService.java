...
@Service
final class DentalEvidenceService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    DentalEvidenceService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    DentalEvidenceWire create(
            ClinicalIdentity identity, String idempotencyKey, DentalEvidenceCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "DENTAL_EVIDENCE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into dental_evidence_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "DENTAL_EVIDENCE_CREATED", "DentalEvidenceCreated");
            completeCommand(identity, "DENTAL_EVIDENCE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<DentalEvidenceWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from dental_evidence_record
    ...
    private DentalEvidenceWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from dental_evidence_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new DentalEvidenceWire(
                ...
                .optional().orElseThrow(DentalEvidenceService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new DentalEvidenceException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new DentalEvidenceException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'DENTAL_EVIDENCE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'DENTAL_EVIDENCE', :aggregate, :version, :event_type, 1,
    ...
    private static DentalEvidenceException invalid(String message) {
        return new DentalEvidenceException("DENTAL_EVIDENCE_REQUEST_INVALID", 400, message);
    }

    static DentalEvidenceException contextDenied() {
        return new DentalEvidenceException("CONTEXT_NOT_PERMITTED", 403,
