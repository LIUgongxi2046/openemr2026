...
@RestControllerAdvice(assignableTypes = DentalFollowupController.class)
final class DentalFollowupExceptionHandler {
    @ExceptionHandler(DentalFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DentalFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DENTAL_FOLLOWUP_CONSTRAINT_CONFLICT",
