...
import org.openemr2026.contracts.DentalQcReviewCreateRequestWire;
import org.openemr2026.contracts.DentalQcReviewWire;
...
@Service
final class DentalQcReviewService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    DentalQcReviewService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    DentalQcReviewWire record(
            ClinicalIdentity identity, String idempotencyKey, DentalQcReviewCreateRequestWire request) {
        ...
        if (request.reviewConclusion() == DentalQcReviewCreateRequestWire.ReviewConclusionValue.FAIL
                && defectDescription == null) {
            throw new DentalQcReviewException(
                    "DENTAL_QC_DEFECT_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "DENTAL_QC_REVIEW", idempotencyKey,
                    sha256(request.patientId() + "|" + request.reviewedRecordType() + "|" + request.reviewedRecordId()));
            UUID reviewId = UUID.randomUUID();
            jdbc.sql("""
                    insert into dental_qc_review(
            ...
            appendEvidence(identity, request.patientId(), reviewId, 1, "DENTAL_QC_REVIEWED",
                    "DentalQcReviewed");
            completeCommand(identity, "DENTAL_QC_REVIEW", idempotencyKey, reviewId);
            return review(identity.tenantId(), reviewId);
        });
    }

    List<DentalQcReviewWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select review_id from dental_qc_review
    ...
    private DentalQcReviewWire review(UUID tenantId, UUID reviewId) {
        return jdbc.sql("""
                ...
                .optional().orElseThrow(DentalQcReviewService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new DentalQcReviewException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new DentalQcReviewException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'DENTAL_QC_REVIEW', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'DENTAL_QC_REVIEW', :aggregate, :version, :event_type, 1,
    ...
    private static DentalQcReviewException invalid(String message) {
        return new DentalQcReviewException("DENTAL_QC_REQUEST_INVALID", 400, message);
    }

    static DentalQcReviewException contextDenied() {
        return new DentalQcReviewException(
                "CONTEXT_NOT_PERMITTED", 403, "The requested dental QC context is not permitted");
    }
