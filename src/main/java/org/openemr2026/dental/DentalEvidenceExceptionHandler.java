...
@RestControllerAdvice(assignableTypes = DentalEvidenceController.class)
final class DentalEvidenceExceptionHandler {
    @ExceptionHandler(DentalEvidenceException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DentalEvidenceException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DENTAL_EVIDENCE_CONSTRAINT_CONFLICT",
