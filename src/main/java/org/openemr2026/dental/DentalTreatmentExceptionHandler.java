...
@RestControllerAdvice(assignableTypes = DentalTreatmentController.class)
final class DentalTreatmentExceptionHandler {
    @ExceptionHandler(DentalTreatmentException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DentalTreatmentException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DENTAL_TREATMENT_CONSTRAINT_CONFLICT",
                "The dental treatment record conflicts with current facts"));
    }
