...
import org.openemr2026.contracts.DentalFollowupRecordCreateRequestWire;
import org.openemr2026.contracts.DentalFollowupRecordWire;
...
@Service
final class DentalFollowupService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    DentalFollowupService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    DentalFollowupRecordWire record(
            ClinicalIdentity identity, String idempotencyKey, DentalFollowupRecordCreateRequestWire request) {
        ...
        if (!Boolean.TRUE.equals(request.attended()) && noShowReason == null) {
            throw new DentalFollowupException(
                    "DENTAL_FOLLOWUP_NO_SHOW_REASON_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "DENTAL_FOLLOWUP_RECORD", idempotencyKey,
                    sha256(request.patientId() + "|" + followupReason + "|" + request.scheduledDate()));
            UUID followupId = UUID.randomUUID();
            jdbc.sql("""
                    insert into dental_followup_record(
            ...
            appendEvidence(identity, request.patientId(), followupId, 1, "DENTAL_FOLLOWUP_RECORDED",
                    "DentalFollowupRecorded");
            completeCommand(identity, "DENTAL_FOLLOWUP_RECORD", idempotencyKey, followupId);
            return followup(identity.tenantId(), followupId);
        });
    }

    List<DentalFollowupRecordWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select followup_id from dental_followup_record
    ...
    private DentalFollowupRecordWire followup(UUID tenantId, UUID followupId) {
        return jdbc.sql("""
                ...
                .optional().orElseThrow(DentalFollowupService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new DentalFollowupException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new DentalFollowupException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'DENTAL_FOLLOWUP', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'DENTAL_FOLLOWUP', :aggregate, :version, :event_type, 1,
    ...
    private static DentalFollowupException invalid(String message) {
        return new DentalFollowupException("DENTAL_FOLLOWUP_REQUEST_INVALID", 400, message);
    }

    static DentalFollowupException contextDenied() {
        return new DentalFollowupException(
