...
@RestControllerAdvice(assignableTypes = DentalQcReviewController.class)
final class DentalQcReviewExceptionHandler {
    @ExceptionHandler(DentalQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DentalQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DENTAL_QC_CONSTRAINT_CONFLICT",
                "The dental QC review conflicts with current facts"));
    }
