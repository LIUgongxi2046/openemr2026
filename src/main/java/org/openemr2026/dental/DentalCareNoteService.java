...
import org.openemr2026.contracts.DentalCareNoteCreateRequestWire;
import org.openemr2026.contracts.DentalCareNoteWire;
...
@Service
final class DentalCareNoteService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    DentalCareNoteService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    DentalCareNoteWire create(
            ClinicalIdentity identity, String idempotencyKey, DentalCareNoteCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "DENTAL_CARE_NOTE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into dental_care_note(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "DENTAL_CARE_NOTE_CREATED", "DentalCareNoteCreated");
            completeCommand(identity, "DENTAL_CARE_NOTE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<DentalCareNoteWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from dental_care_note
    ...
    private DentalCareNoteWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from dental_care_note where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new DentalCareNoteWire(
                ...
                .optional().orElseThrow(DentalCareNoteService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new DentalCareNoteException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new DentalCareNoteException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'DENTAL_CARE_NOTE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'DENTAL_CARE_NOTE', :aggregate, :version, :event_type, 1,
    ...
    private static DentalCareNoteException invalid(String message) {
        return new DentalCareNoteException("DENTAL_CARE_NOTE_REQUEST_INVALID", 400, message);
    }

    static DentalCareNoteException contextDenied() {
        return new DentalCareNoteException("CONTEXT_NOT_PERMITTED", 403,
