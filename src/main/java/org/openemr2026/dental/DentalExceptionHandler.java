...
@RestControllerAdvice(assignableTypes = DentalController.class)
final class DentalExceptionHandler {
    @ExceptionHandler(DentalException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DentalException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DENTAL_CONSTRAINT_CONFLICT", "The dental record command conflicts with current facts"));
    }
