...
@RestControllerAdvice(assignableTypes = EmergencyPreadmissionController.class)
final class EmergencyPreadmissionExceptionHandler {
    @ExceptionHandler(EmergencyPreadmissionException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EmergencyPreadmissionException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "EMERGENCY_PREADMISSION_CONSTRAINT_CONFLICT",
                "The emergency preadmission command conflicts with current facts"));
    }
