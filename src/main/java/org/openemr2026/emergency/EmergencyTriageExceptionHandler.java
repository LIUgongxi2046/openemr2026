...
@RestControllerAdvice(assignableTypes = EmergencyTriageController.class)
final class EmergencyTriageExceptionHandler {
    @ExceptionHandler(EmergencyTriageException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EmergencyTriageException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "EMERGENCY_TRIAGE_CONSTRAINT_CONFLICT", "The emergency triage command conflicts with current facts"));
    }
