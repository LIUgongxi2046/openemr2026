...
@RestControllerAdvice(assignableTypes = EmergencyIdentityVerificationController.class)
final class EmergencyIdentityVerificationExceptionHandler {
    @ExceptionHandler(EmergencyIdentityVerificationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EmergencyIdentityVerificationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "EMERGENCY_IDENTITY_CONSTRAINT_CONFLICT",
                "The identity verification conflicts with the current emergency encounter"));
    }
