...
@RestControllerAdvice(assignableTypes = EmergencyObservationController.class)
final class EmergencyObservationExceptionHandler {
    @ExceptionHandler(EmergencyObservationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EmergencyObservationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "EMERGENCY_OBSERVATION_CONSTRAINT_CONFLICT",
                "The emergency observation command conflicts with current facts"));
    }
