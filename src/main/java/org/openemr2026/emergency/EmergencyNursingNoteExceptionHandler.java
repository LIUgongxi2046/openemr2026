...
@RestControllerAdvice(assignableTypes = EmergencyNursingNoteController.class)
final class EmergencyNursingNoteExceptionHandler {
    @ExceptionHandler(EmergencyNursingNoteException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EmergencyNursingNoteException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "EMERGENCY_NURSING_NOTE_CONSTRAINT_CONFLICT",
