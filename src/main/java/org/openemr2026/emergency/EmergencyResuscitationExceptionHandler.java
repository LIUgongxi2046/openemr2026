...
@RestControllerAdvice(assignableTypes = EmergencyResuscitationController.class)
final class EmergencyResuscitationExceptionHandler {
    @ExceptionHandler(EmergencyResuscitationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EmergencyResuscitationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "EMERGENCY_RESUSCITATION_CONSTRAINT_CONFLICT",
                "The emergency resuscitation command conflicts with current facts"));
    }
