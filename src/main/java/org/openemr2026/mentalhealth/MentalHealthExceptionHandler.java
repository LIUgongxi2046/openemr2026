...
@RestControllerAdvice(assignableTypes = MentalHealthController.class)
final class MentalHealthExceptionHandler {
    @ExceptionHandler(MentalHealthException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(MentalHealthException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "MENTAL_HEALTH_CONSTRAINT_CONFLICT", "The mental health record command conflicts with current facts"));
    }
