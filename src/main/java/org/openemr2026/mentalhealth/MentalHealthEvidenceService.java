...
import org.openemr2026.contracts.MentalHealthEvidenceCreateRequestWire;
import org.openemr2026.contracts.MentalHealthEvidenceWire;
...
@Service
final class MentalHealthEvidenceService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    MentalHealthEvidenceService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    MentalHealthEvidenceWire create(
            ClinicalIdentity identity, String idempotencyKey, MentalHealthEvidenceCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "MENTAL_HEALTH_EVIDENCE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into mental_health_evidence_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "MENTAL_HEALTH_EVIDENCE_CREATED", "MentalHealthEvidenceCreated");
            completeCommand(identity, "MENTAL_HEALTH_EVIDENCE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<MentalHealthEvidenceWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from mental_health_evidence_record
    ...
    private MentalHealthEvidenceWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from mental_health_evidence_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new MentalHealthEvidenceWire(
                ...
                .optional().orElseThrow(MentalHealthEvidenceService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new MentalHealthEvidenceException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new MentalHealthEvidenceException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'MENTAL_HEALTH_EVIDENCE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'MENTAL_HEALTH_EVIDENCE', :aggregate, :version, :event_type, 1,
    ...
    private static MentalHealthEvidenceException invalid(String message) {
        return new MentalHealthEvidenceException("MENTAL_HEALTH_EVIDENCE_REQUEST_INVALID", 400, message);
    }

    static MentalHealthEvidenceException contextDenied() {
        return new MentalHealthEvidenceException("CONTEXT_NOT_PERMITTED", 403,
