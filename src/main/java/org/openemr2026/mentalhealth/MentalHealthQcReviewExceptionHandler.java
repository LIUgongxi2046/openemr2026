...
@RestControllerAdvice(assignableTypes = MentalHealthQcReviewController.class)
final class MentalHealthQcReviewExceptionHandler {
    @ExceptionHandler(MentalHealthQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(MentalHealthQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "MENTAL_HEALTH_QC_CONSTRAINT_CONFLICT",
                "The mental QC review conflicts with current facts"));
    }
