...
@RestControllerAdvice(assignableTypes = MentalHealthTreatmentController.class)
final class MentalHealthTreatmentExceptionHandler {
    @ExceptionHandler(MentalHealthTreatmentException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(MentalHealthTreatmentException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "MENTAL_HEALTH_TREATMENT_CONSTRAINT_CONFLICT",
                "The emergency nursing note command conflicts with current facts"));
    }
