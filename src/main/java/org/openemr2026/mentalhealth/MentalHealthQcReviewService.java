...
import org.openemr2026.contracts.MentalHealthQcReviewCreateRequestWire;
import org.openemr2026.contracts.MentalHealthQcReviewWire;
...
@Service
final class MentalHealthQcReviewService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    MentalHealthQcReviewService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    MentalHealthQcReviewWire record(
            ClinicalIdentity identity, String idempotencyKey, MentalHealthQcReviewCreateRequestWire request) {
        ...
        if (request.reviewConclusion() == MentalHealthQcReviewCreateRequestWire.ReviewConclusionValue.FAIL
                && defectDescription == null) {
            throw new MentalHealthQcReviewException(
                    "MENTAL_HEALTH_QC_DEFECT_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "MENTAL_HEALTH_QC_REVIEW", idempotencyKey,
                    sha256(request.patientId() + "|" + request.reviewedRecordType() + "|" + request.reviewedRecordId()));
            UUID reviewId = UUID.randomUUID();
            jdbc.sql("""
                    insert into mental_health_qc_review(
            ...
            appendEvidence(identity, request.patientId(), reviewId, 1, "MENTAL_HEALTH_QC_REVIEWED",
                    "MentalHealthQcReviewed");
            completeCommand(identity, "MENTAL_HEALTH_QC_REVIEW", idempotencyKey, reviewId);
            return review(identity.tenantId(), reviewId);
        });
    }

    List<MentalHealthQcReviewWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select review_id from mental_health_qc_review
    ...
    private MentalHealthQcReviewWire review(UUID tenantId, UUID reviewId) {
        return jdbc.sql("""
                ...
                from mental_health_qc_review
                where tenant_id = :tenant and review_id = :review
                """).param("tenant", tenantId).param("review", reviewId)
                .query((rs, row) -> new MentalHealthQcReviewWire(
                        rs.getObject("review_id", UUID.class),
                        rs.getObject("patient_id", UUID.class),
                        rs.getObject("encounter_id", UUID.class),
                        rs.getObject("facility_id", UUID.class),
                        MentalHealthQcReviewWire.ReviewedRecordTypeValue.valueOf(rs.getString("reviewed_record_type")),
                        rs.getObject("reviewed_record_id", UUID.class),
                        MentalHealthQcReviewWire.ReviewConclusionValue.valueOf(rs.getString("review_conclusion")),
                ...
                .optional().orElseThrow(MentalHealthQcReviewService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new MentalHealthQcReviewException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new MentalHealthQcReviewException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'MENTAL_HEALTH_QC_REVIEW', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'MENTAL_HEALTH_QC_REVIEW', :aggregate, :version, :event_type, 1,
    ...
    private static MentalHealthQcReviewException invalid(String message) {
        return new MentalHealthQcReviewException("MENTAL_HEALTH_QC_REQUEST_INVALID", 400, message);
    }

    static MentalHealthQcReviewException contextDenied() {
        return new MentalHealthQcReviewException(
                "CONTEXT_NOT_PERMITTED", 403, "The requested mental QC context is not permitted");
    }
