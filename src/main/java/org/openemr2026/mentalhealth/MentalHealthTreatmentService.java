...
@Service
final class MentalHealthTreatmentService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    MentalHealthTreatmentService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    MentalHealthTreatmentWire create(
            ClinicalIdentity identity, String idempotencyKey, MentalHealthTreatmentCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "MENTAL_HEALTH_TREATMENT_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into mental_health_treatment_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "MENTAL_HEALTH_TREATMENT_CREATED", "MentalHealthTreatmentCreated");
            completeCommand(identity, "MENTAL_HEALTH_TREATMENT_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<MentalHealthTreatmentWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from mental_health_treatment_record
    ...
    private MentalHealthTreatmentWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from mental_health_treatment_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new MentalHealthTreatmentWire(
                ...
                .optional().orElseThrow(MentalHealthTreatmentService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new MentalHealthTreatmentException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new MentalHealthTreatmentException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'MENTAL_HEALTH_TREATMENT', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'MENTAL_HEALTH_TREATMENT', :aggregate, :version, :event_type, 1,
    ...
    private static MentalHealthTreatmentException invalid(String message) {
        return new MentalHealthTreatmentException("MENTAL_HEALTH_TREATMENT_REQUEST_INVALID", 400, message);
    }

    static MentalHealthTreatmentException contextDenied() {
        return new MentalHealthTreatmentException("CONTEXT_NOT_PERMITTED", 403,
