...
@RestControllerAdvice(assignableTypes = MentalHealthCrisisFollowupController.class)
final class MentalHealthCrisisFollowupExceptionHandler {
    @ExceptionHandler(MentalHealthCrisisFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(MentalHealthCrisisFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "MENTAL_HEALTH_CRISIS_FOLLOWUP_CONSTRAINT_CONFLICT",
                "The mental health crisis followup conflicts with current facts"));
    }
