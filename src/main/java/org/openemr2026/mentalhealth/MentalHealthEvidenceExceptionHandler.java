...
@RestControllerAdvice(assignableTypes = MentalHealthEvidenceController.class)
final class MentalHealthEvidenceExceptionHandler {
    @ExceptionHandler(MentalHealthEvidenceException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(MentalHealthEvidenceException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "MENTAL_HEALTH_EVIDENCE_CONSTRAINT_CONFLICT",
