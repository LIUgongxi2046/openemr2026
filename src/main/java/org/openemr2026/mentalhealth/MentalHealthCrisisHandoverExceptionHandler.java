...
@RestControllerAdvice(assignableTypes = MentalHealthCrisisHandoverController.class)
final class MentalHealthCrisisHandoverExceptionHandler {
    @ExceptionHandler(MentalHealthCrisisHandoverException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(MentalHealthCrisisHandoverException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "MENTAL_HEALTH_CRISIS_CONSTRAINT_CONFLICT",
                "The mental health crisis handover conflicts with current facts"));
    }
