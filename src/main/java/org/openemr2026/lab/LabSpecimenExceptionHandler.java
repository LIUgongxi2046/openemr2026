...
@RestControllerAdvice(assignableTypes = LabSpecimenController.class)
final class LabSpecimenExceptionHandler {
    @ExceptionHandler(LabSpecimenException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(LabSpecimenException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "LAB_SPECIMEN_CONSTRAINT_CONFLICT", "The lab specimen command conflicts with current facts"));
    }
