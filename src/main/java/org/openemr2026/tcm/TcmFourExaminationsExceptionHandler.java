...
@RestControllerAdvice(assignableTypes = TcmFourExaminationsController.class)
final class TcmFourExaminationsExceptionHandler {
    @ExceptionHandler(TcmFourExaminationsException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(TcmFourExaminationsException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "TCM_FOUR_EXAMINATIONS_CONSTRAINT_CONFLICT",
                "The TCM four examinations record conflicts with current facts"));
    }
