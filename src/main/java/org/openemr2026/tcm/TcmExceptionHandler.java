...
@RestControllerAdvice(assignableTypes = TcmController.class)
final class TcmExceptionHandler {
    @ExceptionHandler(TcmException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(TcmException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "TCM_CONSTRAINT_CONFLICT", "The TCM record command conflicts with current facts"));
    }
