...
@RestControllerAdvice(assignableTypes = TcmFollowupController.class)
final class TcmFollowupExceptionHandler {
    @ExceptionHandler(TcmFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(TcmFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "TCM_FOLLOWUP_CONSTRAINT_CONFLICT",
