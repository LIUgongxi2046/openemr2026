...
@RestControllerAdvice(assignableTypes = TcmQcReviewController.class)
final class TcmQcReviewExceptionHandler {
    @ExceptionHandler(TcmQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(TcmQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "TCM_QC_CONSTRAINT_CONFLICT",
                "The tcm QC review conflicts with current facts"));
    }
