...
@RestControllerAdvice(assignableTypes = TcmCareNoteController.class)
final class TcmCareNoteExceptionHandler {
    @ExceptionHandler(TcmCareNoteException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(TcmCareNoteException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "TCM_CARE_NOTE_CONSTRAINT_CONFLICT",
