...
@RestControllerAdvice(assignableTypes = TcmHerbalPrescriptionController.class)
final class TcmHerbalPrescriptionExceptionHandler {
    @ExceptionHandler(TcmHerbalPrescriptionException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(TcmHerbalPrescriptionException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "TCM_PRESCRIPTION_CONSTRAINT_CONFLICT",
                "The TCM herbal prescription conflicts with current facts"));
    }
