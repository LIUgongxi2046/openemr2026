...
@Service
final class TcmQcReviewService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    TcmQcReviewService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    TcmQcReviewWire record(
            ClinicalIdentity identity, String idempotencyKey, TcmQcReviewCreateRequestWire request) {
        ...
        if (request.reviewConclusion() == TcmQcReviewCreateRequestWire.ReviewConclusionValue.FAIL
                && defectDescription == null) {
            throw new TcmQcReviewException(
                    "TCM_QC_DEFECT_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            ...
            appendEvidence(identity, request.patientId(), reviewId, 1, "TCM_QC_REVIEWED",
                    "TcmQcReviewed");
            completeCommand(identity, "TCM_QC_REVIEW", idempotencyKey, reviewId);
            return review(identity.tenantId(), reviewId);
        });
    }

    List<TcmQcReviewWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select review_id from tcm_qc_review
    ...
    private TcmQcReviewWire review(UUID tenantId, UUID reviewId) {
        return jdbc.sql("""
                ...
                .query((rs, row) -> new TcmQcReviewWire(
                        rs.getObject("review_id", UUID.class),
                        rs.getObject("patient_id", UUID.class),
                        rs.getObject("encounter_id", UUID.class),
                        rs.getObject("facility_id", UUID.class),
                        TcmQcReviewWire.ReviewedRecordTypeValue.valueOf(rs.getString("reviewed_record_type")),
                        rs.getObject("reviewed_record_id", UUID.class),
                        TcmQcReviewWire.ReviewConclusionValue.valueOf(rs.getString("review_conclusion")),
                ...
                .optional().orElseThrow(TcmQcReviewService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new TcmQcReviewException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new TcmQcReviewException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'TCM_QC_REVIEW', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'TCM_QC_REVIEW', :aggregate, :version, :event_type, 1,
    ...
    private static TcmQcReviewException invalid(String message) {
        return new TcmQcReviewException("TCM_QC_REQUEST_INVALID", 400, message);
    }

    static TcmQcReviewException contextDenied() {
        return new TcmQcReviewException(
                "CONTEXT_NOT_PERMITTED", 403, "The requested tcm QC context is not permitted");
    }
