...
@RestControllerAdvice(assignableTypes = DermatologyBiologicScreeningController.class)
final class DermatologyBiologicScreeningExceptionHandler {
    @ExceptionHandler(DermatologyBiologicScreeningException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DermatologyBiologicScreeningException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DERMATOLOGY_BIOLOGIC_CONSTRAINT_CONFLICT",
                "The dermatology biologic screening conflicts with current facts"));
    }
