...
@RestControllerAdvice(assignableTypes = DermatologyController.class)
final class DermatologyExceptionHandler {
    @ExceptionHandler(DermatologyException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DermatologyException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DERMATOLOGY_CONSTRAINT_CONFLICT", "The dermatology record command conflicts with current facts"));
    }
