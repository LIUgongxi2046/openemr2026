...
@Service
final class DermatologyEvidenceService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    DermatologyEvidenceService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    DermatologyEvidenceWire create(
            ClinicalIdentity identity, String idempotencyKey, DermatologyEvidenceCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "DERMATOLOGY_EVIDENCE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into dermatology_evidence_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "DERMATOLOGY_EVIDENCE_CREATED", "DermatologyEvidenceCreated");
            completeCommand(identity, "DERMATOLOGY_EVIDENCE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<DermatologyEvidenceWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from dermatology_evidence_record
    ...
    private DermatologyEvidenceWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from dermatology_evidence_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new DermatologyEvidenceWire(
                ...
                .optional().orElseThrow(DermatologyEvidenceService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new DermatologyEvidenceException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new DermatologyEvidenceException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'DERMATOLOGY_EVIDENCE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'DERMATOLOGY_EVIDENCE', :aggregate, :version, :event_type, 1,
    ...
    private static DermatologyEvidenceException invalid(String message) {
        return new DermatologyEvidenceException("DERMATOLOGY_EVIDENCE_REQUEST_INVALID", 400, message);
    }

    static DermatologyEvidenceException contextDenied() {
        return new DermatologyEvidenceException("CONTEXT_NOT_PERMITTED", 403,
