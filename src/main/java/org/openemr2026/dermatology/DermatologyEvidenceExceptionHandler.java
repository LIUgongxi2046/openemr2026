...
@RestControllerAdvice(assignableTypes = DermatologyEvidenceController.class)
final class DermatologyEvidenceExceptionHandler {
    @ExceptionHandler(DermatologyEvidenceException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DermatologyEvidenceException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DERMATOLOGY_EVIDENCE_CONSTRAINT_CONFLICT",
