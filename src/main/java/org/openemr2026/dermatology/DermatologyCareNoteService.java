...
@Service
final class DermatologyCareNoteService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    DermatologyCareNoteService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    DermatologyCareNoteWire create(
            ClinicalIdentity identity, String idempotencyKey, DermatologyCareNoteCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "DERMATOLOGY_CARE_NOTE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into dermatology_care_note(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "DERMATOLOGY_CARE_NOTE_CREATED", "DermatologyCareNoteCreated");
            completeCommand(identity, "DERMATOLOGY_CARE_NOTE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<DermatologyCareNoteWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from dermatology_care_note
    ...
    private DermatologyCareNoteWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from dermatology_care_note where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new DermatologyCareNoteWire(
                ...
                .optional().orElseThrow(DermatologyCareNoteService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new DermatologyCareNoteException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new DermatologyCareNoteException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'DERMATOLOGY_CARE_NOTE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'DERMATOLOGY_CARE_NOTE', :aggregate, :version, :event_type, 1,
    ...
    private static DermatologyCareNoteException invalid(String message) {
        return new DermatologyCareNoteException("DERMATOLOGY_CARE_NOTE_REQUEST_INVALID", 400, message);
    }

    static DermatologyCareNoteException contextDenied() {
        return new DermatologyCareNoteException("CONTEXT_NOT_PERMITTED", 403,
