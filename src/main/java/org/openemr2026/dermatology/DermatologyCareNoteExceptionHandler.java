...
@RestControllerAdvice(assignableTypes = DermatologyCareNoteController.class)
final class DermatologyCareNoteExceptionHandler {
    @ExceptionHandler(DermatologyCareNoteException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DermatologyCareNoteException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DERMATOLOGY_CARE_NOTE_CONSTRAINT_CONFLICT",
