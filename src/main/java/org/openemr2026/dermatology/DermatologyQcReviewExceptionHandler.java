...
@RestControllerAdvice(assignableTypes = DermatologyQcReviewController.class)
final class DermatologyQcReviewExceptionHandler {
    @ExceptionHandler(DermatologyQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DermatologyQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DERMATOLOGY_QC_CONSTRAINT_CONFLICT",
                "The dermatology QC review conflicts with current facts"));
    }
