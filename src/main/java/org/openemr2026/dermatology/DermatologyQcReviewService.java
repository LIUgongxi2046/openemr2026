...
import org.openemr2026.contracts.DermatologyQcReviewCreateRequestWire;
import org.openemr2026.contracts.DermatologyQcReviewWire;
...
@Service
final class DermatologyQcReviewService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    DermatologyQcReviewService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    DermatologyQcReviewWire record(
            ClinicalIdentity identity, String idempotencyKey, DermatologyQcReviewCreateRequestWire request) {
        ...
        if (request.reviewConclusion() == DermatologyQcReviewCreateRequestWire.ReviewConclusionValue.FAIL
                && defectDescription == null) {
            throw new DermatologyQcReviewException(
                    "DERMATOLOGY_QC_DEFECT_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "DERMATOLOGY_QC_REVIEW", idempotencyKey,
                    sha256(request.patientId() + "|" + request.reviewedRecordType() + "|" + request.reviewedRecordId()));
            UUID reviewId = UUID.randomUUID();
            jdbc.sql("""
                    insert into dermatology_qc_review(
            ...
            appendEvidence(identity, request.patientId(), reviewId, 1, "DERMATOLOGY_QC_REVIEWED",
                    "DermatologyQcReviewed");
            completeCommand(identity, "DERMATOLOGY_QC_REVIEW", idempotencyKey, reviewId);
            return review(identity.tenantId(), reviewId);
        });
    }

    List<DermatologyQcReviewWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select review_id from dermatology_qc_review
    ...
    private DermatologyQcReviewWire review(UUID tenantId, UUID reviewId) {
        return jdbc.sql("""
                ...
                from dermatology_qc_review
                where tenant_id = :tenant and review_id = :review
                """).param("tenant", tenantId).param("review", reviewId)
                .query((rs, row) -> new DermatologyQcReviewWire(
                        rs.getObject("review_id", UUID.class),
                        rs.getObject("patient_id", UUID.class),
                        rs.getObject("encounter_id", UUID.class),
                        rs.getObject("facility_id", UUID.class),
                        DermatologyQcReviewWire.ReviewedRecordTypeValue.valueOf(rs.getString("reviewed_record_type")),
                        rs.getObject("reviewed_record_id", UUID.class),
                        DermatologyQcReviewWire.ReviewConclusionValue.valueOf(rs.getString("review_conclusion")),
                ...
                .optional().orElseThrow(DermatologyQcReviewService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new DermatologyQcReviewException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new DermatologyQcReviewException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'DERMATOLOGY_QC_REVIEW', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'DERMATOLOGY_QC_REVIEW', :aggregate, :version, :event_type, 1,
    ...
    private static DermatologyQcReviewException invalid(String message) {
        return new DermatologyQcReviewException("DERMATOLOGY_QC_REQUEST_INVALID", 400, message);
    }

    static DermatologyQcReviewException contextDenied() {
        return new DermatologyQcReviewException(
                "CONTEXT_NOT_PERMITTED", 403, "The requested dermatology QC context is not permitted");
    }
