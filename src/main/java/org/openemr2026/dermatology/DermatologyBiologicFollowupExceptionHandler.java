...
@RestControllerAdvice(assignableTypes = DermatologyBiologicFollowupController.class)
final class DermatologyBiologicFollowupExceptionHandler {
    @ExceptionHandler(DermatologyBiologicFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DermatologyBiologicFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DERMATOLOGY_BIOLOGIC_FOLLOWUP_CONSTRAINT_CONFLICT",
                "The dermatology biologic followup conflicts with current facts"));
    }
