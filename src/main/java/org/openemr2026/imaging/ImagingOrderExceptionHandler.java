...
@RestControllerAdvice(assignableTypes = ImagingOrderController.class)
final class ImagingOrderExceptionHandler {
    @ExceptionHandler(ImagingOrderException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ImagingOrderException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "IMAGING_ORDER_CONSTRAINT_CONFLICT", "The imaging order command conflicts with current facts"));
    }
