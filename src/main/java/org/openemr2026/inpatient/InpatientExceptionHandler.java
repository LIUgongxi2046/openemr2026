...
@RestControllerAdvice(assignableTypes = {
        InpatientController.class, InpatientConsultationController.class, InpatientPathwayController.class})
final class InpatientExceptionHandler {

    @ExceptionHandler(InpatientException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(InpatientException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "INPATIENT_CONSTRAINT_CONFLICT", "The inpatient command conflicts with the current ward state"));
    }
