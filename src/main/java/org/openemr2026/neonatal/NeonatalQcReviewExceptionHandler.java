...
@RestControllerAdvice(assignableTypes = NeonatalQcReviewController.class)
final class NeonatalQcReviewExceptionHandler {
    @ExceptionHandler(NeonatalQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(NeonatalQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "NEONATAL_QC_CONSTRAINT_CONFLICT",
                "The neonatal QC review conflicts with current facts"));
    }
