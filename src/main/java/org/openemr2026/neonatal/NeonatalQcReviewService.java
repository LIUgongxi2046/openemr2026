...
@Service
final class NeonatalQcReviewService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    NeonatalQcReviewService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    NeonatalQcReviewWire record(
            ClinicalIdentity identity, String idempotencyKey, NeonatalQcReviewCreateRequestWire request) {
        ...
        if (request.reviewConclusion() == NeonatalQcReviewCreateRequestWire.ReviewConclusionValue.FAIL
                && defectDescription == null) {
            throw new NeonatalQcReviewException(
                    "NEONATAL_QC_DEFECT_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "NEONATAL_QC_REVIEW", idempotencyKey,
                    sha256(request.patientId() + "|" + request.reviewedRecordType() + "|" + request.reviewedRecordId()));
            UUID reviewId = UUID.randomUUID();
            jdbc.sql("""
                    insert into neonatal_qc_review(
            ...
            appendEvidence(identity, request.patientId(), reviewId, 1, "NEONATAL_QC_REVIEWED",
                    "NeonatalQcReviewed");
            completeCommand(identity, "NEONATAL_QC_REVIEW", idempotencyKey, reviewId);
            return review(identity.tenantId(), reviewId);
        });
    }

    List<NeonatalQcReviewWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select review_id from neonatal_qc_review
    ...
    private NeonatalQcReviewWire review(UUID tenantId, UUID reviewId) {
        return jdbc.sql("""
                ...
                .optional().orElseThrow(NeonatalQcReviewService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new NeonatalQcReviewException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new NeonatalQcReviewException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'NEONATAL_QC_REVIEW', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'NEONATAL_QC_REVIEW', :aggregate, :version, :event_type, 1,
    ...
    private static NeonatalQcReviewException invalid(String message) {
        return new NeonatalQcReviewException("NEONATAL_QC_REQUEST_INVALID", 400, message);
    }

    static NeonatalQcReviewException contextDenied() {
        return new NeonatalQcReviewException(
                "CONTEXT_NOT_PERMITTED", 403, "The requested neonatal QC context is not permitted");
    }
