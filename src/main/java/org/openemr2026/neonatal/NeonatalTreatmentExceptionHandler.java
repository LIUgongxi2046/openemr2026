...
@RestControllerAdvice(assignableTypes = NeonatalTreatmentController.class)
final class NeonatalTreatmentExceptionHandler {
    @ExceptionHandler(NeonatalTreatmentException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(NeonatalTreatmentException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "NEONATAL_TREATMENT_CONSTRAINT_CONFLICT",
