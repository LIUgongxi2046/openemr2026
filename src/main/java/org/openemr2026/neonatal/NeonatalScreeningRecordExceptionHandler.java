...
@RestControllerAdvice(assignableTypes = NeonatalScreeningRecordController.class)
final class NeonatalScreeningRecordExceptionHandler {
    @ExceptionHandler(NeonatalScreeningRecordException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(NeonatalScreeningRecordException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "NEONATAL_SCREENING_CONSTRAINT_CONFLICT",
                "The neonatal screening record conflicts with current facts"));
    }
