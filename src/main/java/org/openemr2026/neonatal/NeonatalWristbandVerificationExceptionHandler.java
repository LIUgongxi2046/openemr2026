...
@RestControllerAdvice(assignableTypes = NeonatalWristbandVerificationController.class)
final class NeonatalWristbandVerificationExceptionHandler {
    @ExceptionHandler(NeonatalWristbandVerificationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(NeonatalWristbandVerificationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "NEONATAL_WRISTBAND_CONSTRAINT_CONFLICT",
                "The neonatal wristband verification conflicts with current facts"));
    }
