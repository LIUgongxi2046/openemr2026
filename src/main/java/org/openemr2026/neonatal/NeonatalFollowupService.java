...
import org.openemr2026.contracts.NeonatalFollowupRecordCreateRequestWire;
import org.openemr2026.contracts.NeonatalFollowupRecordWire;
...
@Service
final class NeonatalFollowupService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    NeonatalFollowupService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    NeonatalFollowupRecordWire record(
            ClinicalIdentity identity, String idempotencyKey, NeonatalFollowupRecordCreateRequestWire request) {
        ...
        if (!Boolean.TRUE.equals(request.attended()) && noShowReason == null) {
            throw new NeonatalFollowupException(
                    "NEONATAL_FOLLOWUP_NO_SHOW_REASON_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "NEONATAL_FOLLOWUP_RECORD", idempotencyKey,
                    sha256(request.patientId() + "|" + followupReason + "|" + request.scheduledDate()));
            UUID followupId = UUID.randomUUID();
            jdbc.sql("""
                    insert into neonatal_followup_record(
            ...
            appendEvidence(identity, request.patientId(), followupId, 1, "NEONATAL_FOLLOWUP_RECORDED",
                    "NeonatalFollowupRecorded");
            completeCommand(identity, "NEONATAL_FOLLOWUP_RECORD", idempotencyKey, followupId);
            return followup(identity.tenantId(), followupId);
        });
    }

    List<NeonatalFollowupRecordWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select followup_id from neonatal_followup_record
    ...
    private NeonatalFollowupRecordWire followup(UUID tenantId, UUID followupId) {
        return jdbc.sql("""
                ...
                .optional().orElseThrow(NeonatalFollowupService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new NeonatalFollowupException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new NeonatalFollowupException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'NEONATAL_FOLLOWUP', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'NEONATAL_FOLLOWUP', :aggregate, :version, :event_type, 1,
    ...
    private static NeonatalFollowupException invalid(String message) {
        return new NeonatalFollowupException("NEONATAL_FOLLOWUP_REQUEST_INVALID", 400, message);
    }

    static NeonatalFollowupException contextDenied() {
        return new NeonatalFollowupException(
