...
@RestControllerAdvice(assignableTypes = NeonatalController.class)
final class NeonatalExceptionHandler {
    @ExceptionHandler(NeonatalException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(NeonatalException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "NEONATAL_CONSTRAINT_CONFLICT", "The neonatal record command conflicts with current facts"));
    }
