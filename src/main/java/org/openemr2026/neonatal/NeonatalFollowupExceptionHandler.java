...
@RestControllerAdvice(assignableTypes = NeonatalFollowupController.class)
final class NeonatalFollowupExceptionHandler {
    @ExceptionHandler(NeonatalFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(NeonatalFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "NEONATAL_FOLLOWUP_CONSTRAINT_CONFLICT",
