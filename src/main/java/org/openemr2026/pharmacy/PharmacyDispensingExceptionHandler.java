...
@RestControllerAdvice(assignableTypes = PharmacyDispensingController.class)
final class PharmacyDispensingExceptionHandler {
    @ExceptionHandler(PharmacyDispensingException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(PharmacyDispensingException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PHARMACY_DISPENSING_CONSTRAINT_CONFLICT", "The pharmacy dispensing command conflicts with current facts"));
    }
