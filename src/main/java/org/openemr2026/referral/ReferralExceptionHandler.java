...
final class ReferralExceptionHandler {
    @ExceptionHandler(ReferralException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ReferralException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "REFERRAL_CONSTRAINT_CONFLICT", "The referral command conflicts with current facts"));
    }
