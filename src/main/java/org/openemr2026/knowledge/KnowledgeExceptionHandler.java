...
@RestControllerAdvice(assignableTypes = { KnowledgeSourceController.class, KnowledgeDocumentController.class,
        KnowledgeSearchController.class })
final class KnowledgeExceptionHandler {
    @ExceptionHandler(KnowledgeException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(KnowledgeException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "KNOWLEDGE_CONSTRAINT_CONFLICT", "The knowledge command conflicts with current facts"));
    }
