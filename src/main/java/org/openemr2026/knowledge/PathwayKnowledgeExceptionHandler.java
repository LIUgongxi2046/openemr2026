...
@RestControllerAdvice(assignableTypes = PathwayKnowledgeController.class)
final class PathwayKnowledgeExceptionHandler {
    @ExceptionHandler(PathwayKnowledgeException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(PathwayKnowledgeException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PATHWAY_KNOWLEDGE_CONSTRAINT_CONFLICT", "The pathway knowledge command conflicts with current facts"));
    }
