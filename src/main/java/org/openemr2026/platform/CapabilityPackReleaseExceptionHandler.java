...
@RestControllerAdvice(assignableTypes = CapabilityPackReleaseController.class)
final class CapabilityPackReleaseExceptionHandler {
    @ExceptionHandler(CapabilityPackReleaseException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(CapabilityPackReleaseException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "CAPABILITY_PACK_RELEASE_CONSTRAINT_CONFLICT",
                "The capability pack release conflicts with current facts"));
    }
