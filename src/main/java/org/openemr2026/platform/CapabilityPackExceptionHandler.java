...
@RestControllerAdvice(assignableTypes = CapabilityPackController.class)
final class CapabilityPackExceptionHandler {
    @ExceptionHandler(CapabilityPackException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(CapabilityPackException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "CAPABILITY_PACK_CONSTRAINT_CONFLICT", "The capability pack command conflicts with current facts"));
    }
