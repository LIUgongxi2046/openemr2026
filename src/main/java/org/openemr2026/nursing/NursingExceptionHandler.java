...
@RestControllerAdvice(assignableTypes = NursingController.class)
final class NursingExceptionHandler {
    @ExceptionHandler(NursingException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(NursingException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "VITAL_SIGN_CONSTRAINT_CONFLICT", "The vital sign record conflicts with current clinical facts"));
    }
