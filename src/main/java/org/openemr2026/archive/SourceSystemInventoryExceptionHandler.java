...
@RestControllerAdvice(assignableTypes = SourceSystemInventoryController.class)
final class SourceSystemInventoryExceptionHandler {
    @ExceptionHandler(SourceSystemInventoryException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(SourceSystemInventoryException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "SOURCE_SYSTEM_CONSTRAINT_CONFLICT",
                "The source system conflicts with current facts"));
    }
