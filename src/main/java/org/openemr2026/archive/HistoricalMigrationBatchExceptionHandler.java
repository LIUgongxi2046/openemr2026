...
@RestControllerAdvice(assignableTypes = HistoricalMigrationBatchController.class)
final class HistoricalMigrationBatchExceptionHandler {
    @ExceptionHandler(HistoricalMigrationBatchException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(HistoricalMigrationBatchException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "HISTORICAL_MIGRATION_CONSTRAINT_CONFLICT",
                "The historical migration batch conflicts with current facts"));
    }
