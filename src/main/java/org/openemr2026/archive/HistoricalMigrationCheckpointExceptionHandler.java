...
@RestControllerAdvice(assignableTypes = HistoricalMigrationCheckpointController.class)
final class HistoricalMigrationCheckpointExceptionHandler {
    @ExceptionHandler(HistoricalMigrationCheckpointException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(HistoricalMigrationCheckpointException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "HISTORICAL_MIGRATION_CHECKPOINT_CONFLICT",
                "The historical migration checkpoint conflicts with current facts"));
    }
