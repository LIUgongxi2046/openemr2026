...
@RestControllerAdvice(assignableTypes = SourceFieldMappingController.class)
final class SourceFieldMappingExceptionHandler {
    @ExceptionHandler(SourceFieldMappingException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(SourceFieldMappingException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "SOURCE_FIELD_MAPPING_CONFLICT",
                "The source field mapping conflicts with current facts"));
    }
