...
@RestControllerAdvice(assignableTypes = MedicalRecordAssetController.class)
final class MedicalRecordAssetExceptionHandler {
    @ExceptionHandler(MedicalRecordAssetException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(MedicalRecordAssetException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "MEDICAL_RECORD_ASSET_CONSTRAINT_CONFLICT",
                "The medical record asset command conflicts with current facts"));
    }
