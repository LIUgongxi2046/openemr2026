...
@RestControllerAdvice(assignableTypes = SourcePatientMatchCandidateController.class)
final class SourcePatientMatchCandidateExceptionHandler {
    @ExceptionHandler(SourcePatientMatchCandidateException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(SourcePatientMatchCandidateException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "PATIENT_MATCH_CONSTRAINT_CONFLICT",
                "The patient match candidate conflicts with current facts"));
    }
