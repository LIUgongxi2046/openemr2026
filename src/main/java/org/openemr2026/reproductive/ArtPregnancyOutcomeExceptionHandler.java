...
@RestControllerAdvice(assignableTypes = ArtPregnancyOutcomeController.class)
final class ArtPregnancyOutcomeExceptionHandler {
    @ExceptionHandler(ArtPregnancyOutcomeException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ArtPregnancyOutcomeException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ART_PREGNANCY_OUTCOME_CONSTRAINT_CONFLICT",
                "The ART pregnancy outcome conflicts with current facts"));
    }
