...
@RestControllerAdvice(assignableTypes = ReproductiveEvidenceController.class)
final class ReproductiveEvidenceExceptionHandler {
    @ExceptionHandler(ReproductiveEvidenceException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ReproductiveEvidenceException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "REPRODUCTIVE_EVIDENCE_CONSTRAINT_CONFLICT",
