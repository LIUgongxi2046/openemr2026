...
@RestControllerAdvice(assignableTypes = ReproductiveQcReviewController.class)
final class ReproductiveQcReviewExceptionHandler {
    @ExceptionHandler(ReproductiveQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ReproductiveQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "REPRODUCTIVE_QC_CONSTRAINT_CONFLICT",
                "The reproductive QC review conflicts with current facts"));
    }
