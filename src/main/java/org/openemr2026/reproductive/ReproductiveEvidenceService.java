...
@Service
final class ReproductiveEvidenceService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    ReproductiveEvidenceService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    ReproductiveEvidenceWire create(
            ClinicalIdentity identity, String idempotencyKey, ReproductiveEvidenceCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "REPRODUCTIVE_EVIDENCE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into reproductive_evidence_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "REPRODUCTIVE_EVIDENCE_CREATED", "ReproductiveEvidenceCreated");
            completeCommand(identity, "REPRODUCTIVE_EVIDENCE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<ReproductiveEvidenceWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from reproductive_evidence_record
    ...
    private ReproductiveEvidenceWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from reproductive_evidence_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new ReproductiveEvidenceWire(
                ...
                .optional().orElseThrow(ReproductiveEvidenceService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new ReproductiveEvidenceException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new ReproductiveEvidenceException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'REPRODUCTIVE_EVIDENCE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'REPRODUCTIVE_EVIDENCE', :aggregate, :version, :event_type, 1,
    ...
    private static ReproductiveEvidenceException invalid(String message) {
        return new ReproductiveEvidenceException("REPRODUCTIVE_EVIDENCE_REQUEST_INVALID", 400, message);
    }

    static ReproductiveEvidenceException contextDenied() {
        return new ReproductiveEvidenceException("CONTEXT_NOT_PERMITTED", 403,
