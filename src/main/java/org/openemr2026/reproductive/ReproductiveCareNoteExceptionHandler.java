...
@RestControllerAdvice(assignableTypes = ReproductiveCareNoteController.class)
final class ReproductiveCareNoteExceptionHandler {
    @ExceptionHandler(ReproductiveCareNoteException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ReproductiveCareNoteException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "REPRODUCTIVE_CARE_NOTE_CONSTRAINT_CONFLICT",
