...
@RestControllerAdvice(assignableTypes = ArtEmbryoTransferController.class)
final class ArtEmbryoTransferExceptionHandler {
    @ExceptionHandler(ArtEmbryoTransferException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ArtEmbryoTransferException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ART_EMBRYO_TRANSFER_CONSTRAINT_CONFLICT",
                "The ART embryo transfer conflicts with current facts"));
    }
