...
@Service
final class ReproductiveQcReviewService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    ReproductiveQcReviewService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    ReproductiveQcReviewWire record(
            ClinicalIdentity identity, String idempotencyKey, ReproductiveQcReviewCreateRequestWire request) {
        ...
        if (request.reviewConclusion() == ReproductiveQcReviewCreateRequestWire.ReviewConclusionValue.FAIL
                && defectDescription == null) {
            throw new ReproductiveQcReviewException(
                    "REPRODUCTIVE_QC_DEFECT_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "REPRODUCTIVE_QC_REVIEW", idempotencyKey,
                    sha256(request.patientId() + "|" + request.reviewedRecordType() + "|" + request.reviewedRecordId()));
            UUID reviewId = UUID.randomUUID();
            jdbc.sql("""
                    insert into reproductive_qc_review(
            ...
            appendEvidence(identity, request.patientId(), reviewId, 1, "REPRODUCTIVE_QC_REVIEWED",
                    "ReproductiveQcReviewed");
            completeCommand(identity, "REPRODUCTIVE_QC_REVIEW", idempotencyKey, reviewId);
            return review(identity.tenantId(), reviewId);
        });
    }

    List<ReproductiveQcReviewWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select review_id from reproductive_qc_review
    ...
    private ReproductiveQcReviewWire review(UUID tenantId, UUID reviewId) {
        return jdbc.sql("""
                ...
                from reproductive_qc_review
                where tenant_id = :tenant and review_id = :review
                """).param("tenant", tenantId).param("review", reviewId)
                .query((rs, row) -> new ReproductiveQcReviewWire(
                        rs.getObject("review_id", UUID.class),
                        rs.getObject("patient_id", UUID.class),
                        rs.getObject("encounter_id", UUID.class),
                        rs.getObject("facility_id", UUID.class),
                        ReproductiveQcReviewWire.ReviewedRecordTypeValue.valueOf(rs.getString("reviewed_record_type")),
                        rs.getObject("reviewed_record_id", UUID.class),
                        ReproductiveQcReviewWire.ReviewConclusionValue.valueOf(rs.getString("review_conclusion")),
                ...
                .optional().orElseThrow(ReproductiveQcReviewService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new ReproductiveQcReviewException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new ReproductiveQcReviewException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'REPRODUCTIVE_QC_REVIEW', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'REPRODUCTIVE_QC_REVIEW', :aggregate, :version, :event_type, 1,
    ...
    private static ReproductiveQcReviewException invalid(String message) {
        return new ReproductiveQcReviewException("REPRODUCTIVE_QC_REQUEST_INVALID", 400, message);
    }

    static ReproductiveQcReviewException contextDenied() {
        return new ReproductiveQcReviewException(
                "CONTEXT_NOT_PERMITTED", 403, "The requested reproductive QC context is not permitted");
    }
