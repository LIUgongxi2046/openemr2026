...
@Service
final class ReproductiveCareNoteService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    ReproductiveCareNoteService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    ReproductiveCareNoteWire create(
            ClinicalIdentity identity, String idempotencyKey, ReproductiveCareNoteCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "REPRODUCTIVE_CARE_NOTE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into reproductive_care_note(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "REPRODUCTIVE_CARE_NOTE_CREATED", "ReproductiveCareNoteCreated");
            completeCommand(identity, "REPRODUCTIVE_CARE_NOTE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<ReproductiveCareNoteWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from reproductive_care_note
    ...
    private ReproductiveCareNoteWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from reproductive_care_note where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new ReproductiveCareNoteWire(
                ...
                .optional().orElseThrow(ReproductiveCareNoteService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new ReproductiveCareNoteException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new ReproductiveCareNoteException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'REPRODUCTIVE_CARE_NOTE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'REPRODUCTIVE_CARE_NOTE', :aggregate, :version, :event_type, 1,
    ...
    private static ReproductiveCareNoteException invalid(String message) {
        return new ReproductiveCareNoteException("REPRODUCTIVE_CARE_NOTE_REQUEST_INVALID", 400, message);
    }

    static ReproductiveCareNoteException contextDenied() {
        return new ReproductiveCareNoteException("CONTEXT_NOT_PERMITTED", 403,
