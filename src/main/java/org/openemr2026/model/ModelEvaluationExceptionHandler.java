...
@RestControllerAdvice(assignableTypes = ModelEvaluationController.class)
final class ModelEvaluationExceptionHandler {
    @ExceptionHandler(ModelEvaluationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ModelEvaluationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "MODEL_EVALUATION_CONSTRAINT_CONFLICT", "The model evaluation command conflicts with current facts"));
    }
