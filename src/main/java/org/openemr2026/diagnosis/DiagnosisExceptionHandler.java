...
final class DiagnosisExceptionHandler {
    @ExceptionHandler(DiagnosisException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(DiagnosisException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "DIAGNOSIS_CONSTRAINT_CONFLICT",
                "The diagnosis command conflicts with the current clinical facts"));
    }
