...
@RestControllerAdvice(assignableTypes = EntQcReviewController.class)
final class EntQcReviewExceptionHandler {
    @ExceptionHandler(EntQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EntQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ENT_QC_CONSTRAINT_CONFLICT",
                "The ent QC review conflicts with current facts"));
    }
