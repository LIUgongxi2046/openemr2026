...
@RestControllerAdvice(assignableTypes = EntAirwayRiskHandoverController.class)
final class EntAirwayRiskHandoverExceptionHandler {
    @ExceptionHandler(EntAirwayRiskHandoverException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EntAirwayRiskHandoverException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ENT_AIRWAY_HANDOVER_CONSTRAINT_CONFLICT",
                "The ENT airway risk handover conflicts with current facts"));
    }
