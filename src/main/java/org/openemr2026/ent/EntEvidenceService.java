...
import org.openemr2026.contracts.EntEvidenceCreateRequestWire;
import org.openemr2026.contracts.EntEvidenceWire;
...
@Service
final class EntEvidenceService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    EntEvidenceService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    EntEvidenceWire create(
            ClinicalIdentity identity, String idempotencyKey, EntEvidenceCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "ENT_EVIDENCE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into ent_evidence_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "ENT_EVIDENCE_CREATED", "EntEvidenceCreated");
            completeCommand(identity, "ENT_EVIDENCE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<EntEvidenceWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from ent_evidence_record
    ...
    private EntEvidenceWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from ent_evidence_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new EntEvidenceWire(
                ...
                .optional().orElseThrow(EntEvidenceService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new EntEvidenceException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new EntEvidenceException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'ENT_EVIDENCE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'ENT_EVIDENCE', :aggregate, :version, :event_type, 1,
    ...
    private static EntEvidenceException invalid(String message) {
        return new EntEvidenceException("ENT_EVIDENCE_REQUEST_INVALID", 400, message);
    }

    static EntEvidenceException contextDenied() {
        return new EntEvidenceException("CONTEXT_NOT_PERMITTED", 403,
