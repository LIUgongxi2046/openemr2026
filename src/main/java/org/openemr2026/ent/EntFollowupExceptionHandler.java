...
@RestControllerAdvice(assignableTypes = EntFollowupController.class)
final class EntFollowupExceptionHandler {
    @ExceptionHandler(EntFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EntFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ENT_FOLLOWUP_CONSTRAINT_CONFLICT",
