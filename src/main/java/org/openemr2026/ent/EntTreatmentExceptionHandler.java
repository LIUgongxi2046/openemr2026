...
@RestControllerAdvice(assignableTypes = EntTreatmentController.class)
final class EntTreatmentExceptionHandler {
    @ExceptionHandler(EntTreatmentException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EntTreatmentException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ENT_TREATMENT_CONSTRAINT_CONFLICT",
