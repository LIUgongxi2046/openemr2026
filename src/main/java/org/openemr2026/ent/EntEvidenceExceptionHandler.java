...
@RestControllerAdvice(assignableTypes = EntEvidenceController.class)
final class EntEvidenceExceptionHandler {
    @ExceptionHandler(EntEvidenceException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EntEvidenceException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ENT_EVIDENCE_CONSTRAINT_CONFLICT",
