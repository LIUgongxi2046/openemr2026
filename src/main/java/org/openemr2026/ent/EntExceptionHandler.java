...
@RestControllerAdvice(assignableTypes = EntController.class)
final class EntExceptionHandler {
    @ExceptionHandler(EntException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(EntException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ENT_CONSTRAINT_CONFLICT", "The ENT record command conflicts with current facts"));
    }
