...
@RestControllerAdvice(assignableTypes = OphthalmologyCareNoteController.class)
final class OphthalmologyCareNoteExceptionHandler {
    @ExceptionHandler(OphthalmologyCareNoteException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(OphthalmologyCareNoteException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OPHTHALMOLOGY_CARE_NOTE_CONSTRAINT_CONFLICT",
