...
@RestControllerAdvice(assignableTypes = OphthalmologyPreopVerificationController.class)
final class OphthalmologyPreopVerificationExceptionHandler {
    @ExceptionHandler(OphthalmologyPreopVerificationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(OphthalmologyPreopVerificationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OPHTHALMOLOGY_PREOP_CONSTRAINT_CONFLICT",
                "The ophthalmology preop verification conflicts with current facts"));
    }
