...
@RestControllerAdvice(assignableTypes = OphthalmologyQcReviewController.class)
final class OphthalmologyQcReviewExceptionHandler {
    @ExceptionHandler(OphthalmologyQcReviewException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(OphthalmologyQcReviewException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OPHTHALMOLOGY_QC_CONSTRAINT_CONFLICT",
                "The ophthalmology QC review conflicts with current facts"));
    }
