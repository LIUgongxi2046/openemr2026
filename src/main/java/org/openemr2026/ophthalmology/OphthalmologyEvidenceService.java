...
@Service
final class OphthalmologyEvidenceService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    OphthalmologyEvidenceService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    OphthalmologyEvidenceWire create(
            ClinicalIdentity identity, String idempotencyKey, OphthalmologyEvidenceCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "OPHTHALMOLOGY_EVIDENCE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into ophthalmology_evidence_record(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "OPHTHALMOLOGY_EVIDENCE_CREATED", "OphthalmologyEvidenceCreated");
            completeCommand(identity, "OPHTHALMOLOGY_EVIDENCE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<OphthalmologyEvidenceWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from ophthalmology_evidence_record
    ...
    private OphthalmologyEvidenceWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from ophthalmology_evidence_record where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new OphthalmologyEvidenceWire(
                ...
                .optional().orElseThrow(OphthalmologyEvidenceService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new OphthalmologyEvidenceException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new OphthalmologyEvidenceException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'OPHTHALMOLOGY_EVIDENCE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'OPHTHALMOLOGY_EVIDENCE', :aggregate, :version, :event_type, 1,
    ...
    private static OphthalmologyEvidenceException invalid(String message) {
        return new OphthalmologyEvidenceException("OPHTHALMOLOGY_EVIDENCE_REQUEST_INVALID", 400, message);
    }

    static OphthalmologyEvidenceException contextDenied() {
        return new OphthalmologyEvidenceException("CONTEXT_NOT_PERMITTED", 403,
