...
@Service
final class OphthalmologyQcReviewService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    OphthalmologyQcReviewService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    OphthalmologyQcReviewWire record(
            ClinicalIdentity identity, String idempotencyKey, OphthalmologyQcReviewCreateRequestWire request) {
        ...
        if (request.reviewConclusion() == OphthalmologyQcReviewCreateRequestWire.ReviewConclusionValue.FAIL
                && defectDescription == null) {
            throw new OphthalmologyQcReviewException(
                    "OPHTHALMOLOGY_QC_DEFECT_REQUIRED", 400,
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "OPHTHALMOLOGY_QC_REVIEW", idempotencyKey,
                    sha256(request.patientId() + "|" + request.reviewedRecordType() + "|" + request.reviewedRecordId()));
            UUID reviewId = UUID.randomUUID();
            jdbc.sql("""
                    insert into ophthalmology_qc_review(
            ...
            appendEvidence(identity, request.patientId(), reviewId, 1, "OPHTHALMOLOGY_QC_REVIEWED",
                    "OphthalmologyQcReviewed");
            completeCommand(identity, "OPHTHALMOLOGY_QC_REVIEW", idempotencyKey, reviewId);
            return review(identity.tenantId(), reviewId);
        });
    }

    List<OphthalmologyQcReviewWire> listRecords(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select review_id from ophthalmology_qc_review
    ...
    private OphthalmologyQcReviewWire review(UUID tenantId, UUID reviewId) {
        return jdbc.sql("""
                ...
                from ophthalmology_qc_review
                where tenant_id = :tenant and review_id = :review
                """).param("tenant", tenantId).param("review", reviewId)
                .query((rs, row) -> new OphthalmologyQcReviewWire(
                        rs.getObject("review_id", UUID.class),
                        rs.getObject("patient_id", UUID.class),
                        rs.getObject("encounter_id", UUID.class),
                        rs.getObject("facility_id", UUID.class),
                        OphthalmologyQcReviewWire.ReviewedRecordTypeValue.valueOf(rs.getString("reviewed_record_type")),
                        rs.getObject("reviewed_record_id", UUID.class),
                        OphthalmologyQcReviewWire.ReviewConclusionValue.valueOf(rs.getString("review_conclusion")),
                ...
                .optional().orElseThrow(OphthalmologyQcReviewService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new OphthalmologyQcReviewException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new OphthalmologyQcReviewException("IDEMPOTENCY_REPLAY", 409,
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'OPHTHALMOLOGY_QC_REVIEW', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'OPHTHALMOLOGY_QC_REVIEW', :aggregate, :version, :event_type, 1,
    ...
    private static OphthalmologyQcReviewException invalid(String message) {
        return new OphthalmologyQcReviewException("OPHTHALMOLOGY_QC_REQUEST_INVALID", 400, message);
    }

    static OphthalmologyQcReviewException contextDenied() {
        return new OphthalmologyQcReviewException(
                "CONTEXT_NOT_PERMITTED", 403, "The requested ophthalmology QC context is not permitted");
    }
