...
@RestControllerAdvice(assignableTypes = OphthalmologyPostopFollowupController.class)
final class OphthalmologyPostopFollowupExceptionHandler {
    @ExceptionHandler(OphthalmologyPostopFollowupException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(OphthalmologyPostopFollowupException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OPHTHALMOLOGY_POSTOP_FOLLOWUP_CONSTRAINT_CONFLICT",
                "The ophthalmology postop followup conflicts with current facts"));
    }
