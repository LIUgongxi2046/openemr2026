...
import org.openemr2026.contracts.OphthalmologyCareNoteCreateRequestWire;
import org.openemr2026.contracts.OphthalmologyCareNoteWire;
...
@Service
final class OphthalmologyCareNoteService {
    private final JdbcClient jdbc;
    private final TransactionTemplate transactions;

    OphthalmologyCareNoteService(JdbcClient jdbc, TransactionTemplate transactions) {
        this.jdbc = jdbc;
        this.transactions = transactions;
    }

    OphthalmologyCareNoteWire create(
            ClinicalIdentity identity, String idempotencyKey, OphthalmologyCareNoteCreateRequestWire request) {
        ...
        return transactions.execute(status -> {
            beginCommand(identity, "OPHTHALMOLOGY_CARE_NOTE_CREATE", idempotencyKey,
                    sha256(request.patientId() + "|" + request.encounterId() + "|" + request.recordedAt()));
            UUID noteId = UUID.randomUUID();
            jdbc.sql("""
                    insert into ophthalmology_care_note(
            ...
            appendEvidence(identity, request.patientId(), noteId, 1,
                    "OPHTHALMOLOGY_CARE_NOTE_CREATED", "OphthalmologyCareNoteCreated");
            completeCommand(identity, "OPHTHALMOLOGY_CARE_NOTE_CREATE", idempotencyKey, noteId);
            return note(identity.tenantId(), noteId, request.patientId());
        });
    }

    List<OphthalmologyCareNoteWire> listNotes(ClinicalIdentity identity, UUID patientId) {
        return jdbc.sql("""
                select note_id from ophthalmology_care_note
    ...
    private OphthalmologyCareNoteWire note(UUID tenantId, UUID noteId, UUID patientId) {
        return jdbc.sql("""
                select note_id, patient_id, encounter_id, facility_id, assessment, intervention,
                  risk_flag, recorded_at, row_version
                from ophthalmology_care_note where tenant_id = :tenant and note_id = :note and patient_id = :patient
                """).param("tenant", tenantId).param("note", noteId).param("patient", patientId)
                .query((rs, row) -> new OphthalmologyCareNoteWire(
                ...
                .optional().orElseThrow(OphthalmologyCareNoteService::contextDenied);
    }
    ...
    private void beginCommand(ClinicalIdentity identity, String scope, String key, String requestHash) {
        if (key == null || key.isBlank() || key.length() > 128) {
            throw new OphthalmologyCareNoteException("INVALID_IDEMPOTENCY_KEY", 400,
        ...
        if (inserted != 1) {
            throw new OphthalmologyCareNoteException("IDEMPOTENCY_REPLAY", 409, "This command key was already used");
        }
    }
    ...
    private void appendEvidence(
        ...
        jdbc.sql("""
                ...
                values (:tenant, :audit, now(), :actor, :action, 'OPHTHALMOLOGY_CARE_NOTE', :resource,
        ...
        jdbc.sql("""
                ...
                values (:tenant, :event, 'OPHTHALMOLOGY_CARE_NOTE', :aggregate, :version, :event_type, 1,
    ...
    private static OphthalmologyCareNoteException invalid(String message) {
        return new OphthalmologyCareNoteException("OPHTHALMOLOGY_CARE_NOTE_REQUEST_INVALID", 400, message);
    }

    static OphthalmologyCareNoteException contextDenied() {
        return new OphthalmologyCareNoteException("CONTEXT_NOT_PERMITTED", 403,
