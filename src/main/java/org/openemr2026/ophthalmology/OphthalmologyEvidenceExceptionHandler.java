...
@RestControllerAdvice(assignableTypes = OphthalmologyEvidenceController.class)
final class OphthalmologyEvidenceExceptionHandler {
    @ExceptionHandler(OphthalmologyEvidenceException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(OphthalmologyEvidenceException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OPHTHALMOLOGY_EVIDENCE_CONSTRAINT_CONFLICT",
