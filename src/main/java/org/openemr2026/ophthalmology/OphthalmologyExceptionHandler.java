...
@RestControllerAdvice(assignableTypes = OphthalmologyController.class)
final class OphthalmologyExceptionHandler {
    @ExceptionHandler(OphthalmologyException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(OphthalmologyException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "OPHTHALMOLOGY_CONSTRAINT_CONFLICT", "The ophthalmology record command conflicts with current facts"));
    }
