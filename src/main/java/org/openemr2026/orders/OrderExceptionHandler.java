...
final class OrderExceptionHandler {
    ...
    ResponseEntity<ApiErrorEnvelope> domainFailure(OrderException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ORDER_CONSTRAINT_CONFLICT", "The order command conflicts with the current clinical facts"));
    }
