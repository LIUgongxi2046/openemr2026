...
@RestControllerAdvice(assignableTypes = ClinicalTaskController.class)
final class ClinicalTaskExceptionHandler {
    @ExceptionHandler(ClinicalTaskException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ClinicalTaskException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "CLINICAL_TASK_CONSTRAINT_CONFLICT", "The task command conflicts with current clinical facts"));
    }
