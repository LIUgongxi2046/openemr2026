...
@RestControllerAdvice(assignableTypes = ClinicalTaskTeamQueueController.class)
final class ClinicalTaskTeamQueueExceptionHandler {
    @ExceptionHandler(ClinicalTaskTeamQueueException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ClinicalTaskTeamQueueException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "CLINICAL_TASK_TEAM_QUEUE_CONSTRAINT_CONFLICT",
                "The clinical task team queue conflicts with current facts"));
    }
