...
@RestControllerAdvice(assignableTypes = WardTransferTaskMigrationController.class)
final class WardTransferTaskMigrationExceptionHandler {
    @ExceptionHandler(WardTransferTaskMigrationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(WardTransferTaskMigrationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "WARD_TASK_MIGRATION_CONSTRAINT_CONFLICT",
                "The ward task migration conflicts with current facts"));
    }
