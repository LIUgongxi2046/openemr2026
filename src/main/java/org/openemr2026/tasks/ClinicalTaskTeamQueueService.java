...
final class ClinicalTaskTeamQueueService {
    ...
    ClinicalTaskTeamQueueWire claim(
        ...
        return transactions.execute(status -> {
            ...
            requireTeamMembership(identity, request.organizationId(), request.facilityId());
            jdbc.sql("""
    ...
    List<ClinicalTaskTeamQueueWire> list(ClinicalIdentity identity, UUID departmentId) {
        return jdbc.sql("""
                select queue_id from clinical_task_team_queue
                where tenant_id = :tenant and department_id = :department
                order by enqueued_at desc, queue_id desc limit 500
                """).param("tenant", identity.tenantId()).param("department", departmentId)
    ...
    private void requireTeamMembership(ClinicalIdentity identity, UUID organizationId, UUID facilityId) {
        ...
        if (count < 1) {
            throw new ClinicalTaskTeamQueueException(
                    "TEAM_MEMBERSHIP_REQUIRED", 403, "No active role can claim a task in this facility");
        }
    }

    private QueueHead lockQueue(UUID tenantId, UUID queueId) {
        return jdbc.sql("""
                select queue_status, row_version from clinical_task_team_queue
                where tenant_id = :tenant and queue_id = :queue for update
                """).param("tenant", tenantId).param("queue", queueId)
                .query((rs, row) -> new QueueHead(rs.getString("queue_status"), rs.getLong("row_version")))
                .optional().orElseThrow(ClinicalTaskTeamQueueService::contextDenied);
    }
    ...
    private record QueueHead(String status, long rowVersion) {}
