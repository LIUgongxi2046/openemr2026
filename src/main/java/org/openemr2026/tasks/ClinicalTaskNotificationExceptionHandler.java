...
@RestControllerAdvice(assignableTypes = ClinicalTaskNotificationController.class)
final class ClinicalTaskNotificationExceptionHandler {
    @ExceptionHandler(ClinicalTaskNotificationException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ClinicalTaskNotificationException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "CLINICAL_TASK_NOTIFICATION_CONSTRAINT_CONFLICT",
                "The clinical task notification command conflicts with current facts"));
    }
