...
@RestControllerAdvice(assignableTypes = ActionExecutionController.class)
final class ActionExecutionExceptionHandler {
    @ExceptionHandler(ActionExecutionException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ActionExecutionException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                ...
                "The action execution conflicts with current facts"));
    }
