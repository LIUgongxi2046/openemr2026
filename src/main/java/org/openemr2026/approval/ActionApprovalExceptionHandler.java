...
@RestControllerAdvice(assignableTypes = ActionApprovalController.class)
final class ActionApprovalExceptionHandler {
    @ExceptionHandler(ActionApprovalException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ActionApprovalException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "ACTION_APPROVAL_CONSTRAINT_CONFLICT", "The action approval command conflicts with current facts"));
    }
