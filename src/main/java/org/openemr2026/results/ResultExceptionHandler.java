...
final class ResultExceptionHandler {
    @ExceptionHandler(ResultException.class)
    ResponseEntity<ApiErrorEnvelope> domainFailure(ResultException failure) {
    ...
    ResponseEntity<ApiErrorEnvelope> constraintFailure(DataIntegrityViolationException ignored) {
        return ResponseEntity.status(409).body(error(
                "RESULT_CONSTRAINT_CONFLICT", "The result command conflicts with current clinical facts"));
    }
