...
<template>
  <section class="patient-strip emergency-context emergency-patient-strip" aria-label="当前急诊患者上下文">
    ...
    <div class="divider"></div>
    <div class="emergency-chief-complaint"><b>{{ complaint }}</b><div class="meta">腕带与患者上下文已核验 · 临床事实实时同步</div></div>
    <span class="risk red">{{ levelLabel }}</span>
    <span v-if="currentTriage?.immediate_action_required" class="risk red">绿色通道</span>
