...
const sidePatientOpen = ref(false);
...
const selectedChild = computed(() => selectedAgent.value?.child_agents.find((child) => child.stage_code === selectedStageCode.value));
...
watch([agents, routingQuery.data, () => props.routeId, () => props.open], ([next, routing, routeId, open]) => {
  ...
  selectedStageCode.value = child?.stage_code ?? '';
}, { immediate: true });
...
async function send() {
  ...
  try {
    const patientId = hasPatientContext.value ? patient.current.value.patientId : null; const encounterId = hasPatientContext.value ? patient.current.value.encounterId : null;
    const run = await createMedicalAgentRun(lease, { patientId, encounterId, mainAgentCode: userOverride.value ? agent.main_agent.agent_code : null, stageCode: userOverride.value ? child.stage_code : null, sourceRoute: props.routeId, targetType: patientId ? 'ENCOUNTER' : null, targetId: encounterId ?? null, objective: text, modelDeploymentId: selectedModelId.value, authorizationLevel: authorizationLevel.value, contextScopes: contextScopes.value });
...
async function cancelRun(message: ChatMessage) {
  ...
  try { const patientId = hasPatientContext.value ? patient.current.value.patientId : null; const encounterId = hasPatientContext.value ? patient.current.value.encounterId : null; const latest = await getMedicalAgentRun(lease, patientId, encounterId, message.runId); if (!['QUEUED', 'RUNNING'].includes(latest.state)) { applyRun(message, latest); return; } applyRun(message, await cancelMedicalAgentRun(lease, patientId, encounterId, latest.run_id, latest.row_version)); }
...
function unbindPatient() { patient.unbind(); messages.value = []; draft.value = ''; }
      ...
      <div class="eva-popup-shell">
        ...
        <section class="eva-popup-workspace">
          ...
          <main class="eva-popup-main">
            <div v-if="selectedAgent" class="eva-agent-context"><strong>{{ clinicianAgentName(selectedAgent.main_agent.display_name) }}</strong><span>{{ selectedAgent.main_agent.doctor_facing_summary }}</span></div>
          ...
          <div class="eva-patient-slot" :class="{ side: mode === 'side', open: sidePatientOpen }">
            <button v-if="mode === 'side' && !sidePatientOpen" type="button" class="eva-side-patient-bar" :aria-expanded="false" @click="sidePatientOpen = true"><span>患者上下文</span><strong>{{ patient.current.value.patientId ? patient.current.value.patientName : '未绑定患者' }}</strong><small>{{ patient.current.value.patientSummary }} · {{ patient.current.value.label }}</small><i aria-hidden="true">▴</i></button>
            <EvaPatientPicker v-if="mode !== 'side' || sidePatientOpen" :current="patient.current.value" :defaults="evaDefaultPatientContexts" :results="patient.results.value" :encounters="patient.encounters.value" :selected-patient-id="patient.selectedPatient.value?.patient_id ?? ''" :searching="patient.searching.value" :loading-encounters="patient.loadingEncounters.value" :notice="patient.notice.value" compact @search="patient.search" @select-default="selectDefault" @select-patient="selectSearchPatient" @select-encounter="selectEncounter" @unbind="unbindPatient" />
          </div>
        </section>
      </div>
    </component>
...
<style scoped>
:global(.global-ai-dialog.eva-popup-root) { width: min(1160px,calc(100vw - 32px)); height: min(860px,calc(100dvh - 32px)); max-height: none; }:global(.global-ai-side-panel.eva-popup-root) { width: var(--assistant-side-width); }.eva-popup-resizer { position: absolute; left: 0; top: 0; bottom: 0; width: 8px; padding: 0; border: 0; background: transparent; cursor: ew-resize; z-index: 10; }.eva-popup-resizer:hover, .eva-popup-resizer:active { background: linear-gradient(90deg, rgb(23 105 224 / 30%), transparent); }.eva-popup-shell { display: grid; grid-template-rows: auto minmax(0,1fr); width: 100%; height: 100%; min-height: 0; }.eva-popup-header { display: grid; grid-template-columns: 46px minmax(0,1fr) auto auto 34px; align-items: center; gap: 10px; min-height: 72px; padding: 10px 13px; color: #fff; background: linear-gradient(135deg,#193a59,#1769e0); }.eva-popup-header > img { width: 46px; height: 46px; object-fit: cover; border-radius: 50%; background: #fff; box-shadow: 0 0 0 2px rgb(255 255 255 / 28%); }.eva-popup-header > div { display: grid; gap: 2px; min-width: 0; }.eva-popup-header span { color: #bdd8f5; font-size: 10px; font-weight: 800; letter-spacing: .5px; }.eva-popup-header h2 { margin: 1px 0; color: #fff; font-size: 17px; }.eva-popup-header p { margin: 0; overflow: hidden; color: #d9e8f7; font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.eva-popup-header > b { padding: 6px 8px; border: 1px solid rgb(255 255 255 / 24%); border-radius: 999px; background: rgb(255 255 255 / 8%); font-size: 10px; white-space: nowrap; }.eva-popup-header nav { display: flex; gap: 2px; padding: 2px; border: 1px solid rgb(255 255 255 / 22%); border-radius: 8px; }.eva-popup-header button { height: 28px; padding: 0 8px; color: #fff; border: 0; border-radius: 6px; background: transparent; font-size: 11px; cursor: pointer; }.eva-popup-header nav button.active { color: #173e64; background: #fff; font-weight: 800; }.eva-popup-header .eva-popup-close { width: 34px; height: 34px; font-size: 22px; }.eva-popup-workspace { display: grid; grid-template-columns: auto minmax(0,1fr) 220px; min-width: 0; min-height: 0; overflow: hidden; }.eva-popup-main { display: grid; grid-template-rows: auto auto minmax(180px,1fr) auto; min-width: 0; min-height: 0; }.eva-agent-context { display: grid; gap: 2px; padding: 8px 12px; border-bottom: 1px solid #d8e3ef; background: #f7fafd; }.eva-agent-context strong { color: #24628f; font-size: 10px; }.eva-agent-context span { color: #6b7f91; font-size: 8px; line-height: 1.5; }.eva-popup-thread { display: grid; align-content: start; gap: 9px; min-height: 0; padding: 12px; overflow-y: auto; background: #fbfcfe; }.eva-popup-empty { padding: 24px 12px; color: #697d90; font-size: 12px; text-align: center; }.eva-popup-empty.error { color: #a32632; }.eva-popup-empty.illustrated { display: grid; justify-items: center; gap: 6px; max-width: 560px; margin: auto; }.eva-popup-empty img { width: min(100%,420px); max-height: 170px; object-fit: contain; border-radius: 10px; mix-blend-mode: multiply; }.eva-popup-empty strong { color: #29435d; font-size: 14px; }.eva-popup-empty p { margin: 0; line-height: 1.6; }.eva-popup-message { display: grid; gap: 7px; width: min(90%,650px); padding: 9px 11px; border: 1px solid #d8e3ed; border-radius: 10px; background: #fff; }.eva-popup-message.user { justify-self: end; width: min(78%,560px); border-color: #acd0ee; background: #edf6ff; }.eva-popup-message header { display: flex; justify-content: space-between; gap: 8px; }.eva-popup-message header b { color: #185b83; font-size: 12px; }.eva-popup-message header span { color: #8694a2; font-size: 10px; }.eva-popup-message > p { margin: 0; white-space: pre-wrap; color: #3c5268; font-size: 13px; line-height: 1.65; }.eva-popup-message ol { display: grid; gap: 1px; padding: 1px; margin: 0; border: 1px solid #dce5ed; border-radius: 8px; background: #e8eef3; list-style: none; }.eva-popup-message li { display: grid; grid-template-columns: 20px minmax(0,1fr); align-items: center; gap: 6px; padding: 7px 8px; background: #fff; }.eva-popup-message li i { display: grid; place-items: center; width: 18px; height: 18px; color: #fff; border-radius: 50%; background: #8497aa; font-size: 7px; font-style: normal; }.eva-popup-message li.done i { background: #159783; }.eva-popup-message li.running i { background: #1769e0; }.eva-popup-message li.failed i { background: #c43d45; }.eva-popup-message li span { display: grid; gap: 2px; min-width: 0; }.eva-popup-message li b, .eva-popup-message li small { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.eva-popup-message li b { color: #3b5269; font-size: 11px; }.eva-popup-message li small { color: #81909f; font-size: 10px; }.eva-popup-composer { display: grid; gap: 7px; padding: 10px 12px max(10px,env(safe-area-inset-bottom)); border-top: 1px solid #d8e3ef; background: #fff; }.eva-popup-composer textarea { width: 100%; min-height: 70px; max-height: 145px; padding: 9px 10px; resize: vertical; border: 1px solid #bdcddd; border-radius: 9px; outline: none; font: inherit; font-size: 13px; line-height: 1.6; }.eva-popup-composer textarea:focus { border-color: #4f91d5; box-shadow: 0 0 0 3px rgb(23 105 224 / 10%); }.eva-popup-composer footer { display: flex; align-items: center; gap: 7px; }.eva-popup-composer footer > :first-child { flex: 1; min-width: 0; }.eva-popup-composer footer > div { display: flex; gap: 5px; }
.global-ai-side-panel.eva-popup-root .eva-popup-header { min-height: 56px; grid-template-columns: 40px minmax(0,1fr) auto 32px; gap: 7px; }
.global-ai-side-panel.eva-popup-root .eva-popup-header > div > span { display: none; }
.global-ai-side-panel.eva-popup-root .eva-popup-header h2 { font-size: 15px; }
...
.global-ai-side-panel.eva-popup-root .eva-patient-slot.side { grid-column: 2; grid-row: 2; min-width: 0; border-top: 1px solid #d8e3ef; }
.global-ai-side-panel.eva-popup-root .eva-patient-slot.side :deep(.eva-patient-picker) { width: 100%; max-height: 220px; border-left: 0; border-top: 0; }
.eva-patient-slot { min-width: 0; min-height: 0; }.eva-patient-slot :deep(.eva-patient-picker) { width: 100%; height: 100%; }.eva-side-patient-bar { display: flex; align-items: center; gap: 8px; width: 100%; min-height: 42px; padding: 8px 12px; color: inherit; border: 0; background: #f8fafc; text-align: left; cursor: pointer; }.eva-side-patient-bar span { color: #718397; font-size: 10px; font-weight: 800; }.eva-side-patient-bar strong { color: #244764; font-size: 12px; }.eva-side-patient-bar small { overflow: hidden; color: #617a90; font-size: 10px; text-overflow: ellipsis; white-space: nowrap; }.eva-side-patient-bar i { margin-left: auto; color: #7d90a3; font-size: 12px; font-style: normal; }
.eva-popup-message-actions { display: flex; align-items: center; justify-content: flex-end; gap: 6px; padding-top: 6px; border-top: 1px solid #edf1f5; }.eva-popup-message-actions span { margin-right: auto; color: #7b8998; font-size: 10px; }.eva-popup-message-actions .btn { min-height: 28px; padding: 4px 9px; font-size: 11px; }
