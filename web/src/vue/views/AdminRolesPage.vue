...
<template>
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-head"><div class="page-title"><h1>角色、工作组与职责分离</h1><p>角色表达权限模板，工作组表达协作分派；岗位任期与临床资质独立治理</p></div><div class="head-actions"><button class="btn" type="button" :disabled="query.isFetching.value" @click="scanConflicts">{{ query.isFetching.value ? '扫描中…' : '职责冲突扫描' }}</button><button class="btn primary" type="button" @click="showCreate = !showCreate">{{ showCreate ? '关闭新建' : '新建角色/工作组' }}</button></div></div>
    ...
    <template v-else>
      ...
      <section v-if="showCreate" class="admin-panel admin-form-panel"><header><div><h2>新建角色/工作组</h2><p>目录定义真实写入版本化配置表，发布后再用于人员授权和任务分派。</p></div></header><form class="admin-form compact-admin-form" @submit.prevent="createCatalogItem"><label><span>编码</span><input v-model="form.code" required placeholder="ROLE-CARD-IP" /></label><label><span>名称</span><input v-model="form.name" required placeholder="心内科住院医生" /></label><label><span>类型</span><select v-model="form.objectType"><option value="ROLE">角色</option><option value="WORKGROUP">工作组</option></select></label><label><span>父角色</span><input v-model="form.parent" /></label><label><span>权限/用途</span><input v-model="form.purpose" required /></label><label><span>范围</span><input v-model="form.scope" required /></label><label><span>责任人</span><input v-model="form.owner" required /></label><button class="button primary" :disabled="Boolean(busy)">{{ busy ? '正在保存…' : '保存目录草案' }}</button></form></section>
      <div class="grid admin-list-detail"><section class="card"><div class="toolbar"><input v-model="keyword" class="search" placeholder="角色编码或名称" /><select v-model="roleTypeFilter" class="select"><option value="ALL">全部类型</option><option value="ROLE">业务角色</option><option value="PRIVILEGED">高权角色</option><option value="WORKGROUP">工作组</option></select></div>
        ...
        <div v-else class="admin-table-wrap"><table class="table"><thead><tr><th>编码/名称</th><th>类型</th><th>父角色</th><th>成员</th><th>权限/用途</th><th>范围</th><th>状态</th></tr></thead><tbody>
          <tr v-for="role in roleCatalog" :key="role.code" :class="{ selected: selectedRole?.code === role.code }" @click="selectedRoleCode = role.code"><td><b>{{ role.name }}</b><br><span class="meta">{{ role.code }}</span></td><td>{{ role.objectType === 'WORKGROUP' ? '工作组' : role.privileged ? '高权角色' : '业务角色' }}</td><td>{{ role.parent }}</td><td>{{ role.members.size }}</td><td>{{ role.purpose }}</td><td>{{ [...role.scopes].join('、') || '未限定' }}</td><td><span class="status" :class="role.status === 'ACTIVE' ? role.privileged ? 'amber' : 'green' : 'blue'">{{ role.status === 'ACTIVE' && role.privileged ? '待周期复核' : statusLabel(role.status) }}</span></td></tr>
        </tbody></table></div>
      </section><aside v-if="selectedRole" class="card"><div class="card-head">{{ selectedRole.name }} · 权限风险</div><div class="card-body"><div class="permission-ring"><b>{{ selectedRole.members.size }}</b><span>有效成员</span></div><div class="folder-row">角色责任人<span>{{ selectedRole.owner }}</span></div><div class="folder-row">有效成员<span>{{ selectedRole.members.size }} 人</span></div><div class="folder-row">高风险动作<span>{{ selectedRole.purpose }}</span></div><div class="folder-row">父角色<span>{{ selectedRole.parent }}</span></div><div class="folder-row">授权范围<span>{{ [...selectedRole.scopes].join('、') || '未限定' }}</span></div><div class="folder-row">扫描范围<span>{{ governance.assignmentCount }} 个有效任期</span></div><div v-if="selectedRole.config" class="toolbar-actions lifecycle-actions"><button class="task-action" type="button" :disabled="selectedRole.config.status !== 'DRAFT' || Boolean(busy)" @click="catalogLifecycle('VALIDATE')">校验</button><button class="task-action" type="button" :disabled="selectedRole.config.status !== 'DRAFT' || Boolean(busy)" @click="catalogLifecycle('SUBMIT')">提交审批</button><button class="task-action" type="button" :disabled="selectedRole.config.status !== 'PENDING_APPROVAL' || Boolean(busy)" @click="catalogLifecycle('APPROVE')">独立批准</button><button class="task-action" type="button" :disabled="selectedRole.config.status !== 'APPROVED' || Boolean(busy)" @click="catalogLifecycle('PUBLISH')">发布</button></div><div v-else class="notice info"><div class="notice-title">内置角色</div>该角色来自已生效岗位任期；如需变更目录定义，请先创建对应的版本化角色目录。</div><div v-if="governance.conflicts.length" class="notice hard"><div class="notice-title">发现 {{ governance.conflicts.length }} 项职责冲突</div><p v-for="conflict in governance.conflicts" :key="`${conflict.personId}-${conflict.roleCodes.join('-')}`">{{ conflict.personDisplayName }}（{{ conflict.personCode }}）：{{ conflict.reason }}</p></div><div v-else class="notice info"><div class="notice-title">当前未发现冲突</div>结论来自数据库中的有效角色任期；扫描不会自动撤权。</div></div></aside></div>
    </template>
  </section>
</template>
