...
async function react(transfusion: BloodTransfusionWire, reactionType: Reaction) {
  const lease = leaseQuery.data.value;
    ...
    <template v-else>
      ...
      <div class="admin-layout">
              ...
              <tbody>
                <tr v-for="transfusion in transfusions" :key="transfusion.transfusion_id">
                  ...
                  <td class="admin-actions">
                    <span v-if="!transfusion.reaction_type" class="admin-actions-inline">
                      <button v-for="reaction in reactions" :key="reaction" class="task-action" :disabled="Boolean(busy)" @click="react(transfusion, reaction)">{{ reactionLabels[reaction] }}</button>
                    </span>
        ...
        <section class="admin-panel admin-form-panel">
          <header><div><h2>输血录入</h2><p>血制品、血型、单位号、容量与第二核验人必填。</p></div></header>
