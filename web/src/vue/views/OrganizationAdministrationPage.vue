...
const page = ref(1);
const pageSize = 15;
const panel = ref<'NONE' | 'CREATE' | 'IMPORT' | 'IMPACT'>('NONE');
const selectedUnitId = ref('');
const form = reactive({ unitType: 'DEPARTMENT' as UnitType, code: '', name: '', parentId: '', subtype: '' });
const importText = ref('DEPARTMENT,CARD-REHAB,心脏康复中心,\nWARD,CARD-REHAB-W1,心脏康复一病区,CARD-REHAB');
...
async function importUnits() {
  ...
  try {
    for (const [rawType, code, name, parentCode = '', subtype = ''] of rows) {
      const unitType = rawType?.toUpperCase() as UnitType;
    ...
    notice.value = `导入完成：${created} 个组织单元已逐项写入数据库，并生成审计与 Outbox 证据。`;
    await query.refetch();
  } catch (error) {
...
<template>
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-head"><div class="page-title"><h1>组织机构与工作单元</h1><p>机构、院区、科室、病区和床位的统一版本化层级</p></div><div class="head-actions"><button class="btn" type="button" @click="panel = panel === 'IMPORT' ? 'NONE' : 'IMPORT'">导入组织</button><button class="btn" type="button" @click="panel = panel === 'IMPACT' ? 'NONE' : 'IMPACT'">查看变更影响</button><button class="btn primary" type="button" @click="panel = panel === 'CREATE' ? 'NONE' : 'CREATE'">{{ panel === 'CREATE' ? '关闭新增' : '新建组织节点' }}</button></div></div>
