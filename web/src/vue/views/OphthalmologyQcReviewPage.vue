...
import type { OphthalmologyQcReviewWire } from '../../generated/contracts';
import { createOphthalmologyQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listOphthalmologyQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('OPHTHALMOLOGY_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('OPHTHALMOLOGY_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-qc', 'records'],
  queryFn: () => listOphthalmologyQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'PREOP_VERIFICATION' as OphthalmologyQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as OphthalmologyQcReviewWire['review_conclusion'],
  defectDescription: '',
...
async function createRecord() {
  ...
  try {
    await createOphthalmologyQcReview(lease, {
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'PREOP_VERIFICATION' ? '术前核对' : '术后随访' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="PREOP_VERIFICATION">术前核对</option><option value="POSTOP_FOLLOWUP">术后随访</option>
            </select></label>
