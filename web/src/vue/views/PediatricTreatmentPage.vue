...
import { createPediatricTreatment, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listPediatricTreatments } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-treatment', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('PEDIATRICS_TREATMENT'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-treatment', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('PEDIATRICS_TREATMENT'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-treatment', 'items'],
  queryFn: () => listPediatricTreatments(patientLease.value!),
...
async function createNote() {
  ...
  try {
    await createPediatricTreatment(lease, {
...
<template>
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        <p class="eyebrow">专科中心 / 儿科</p>
        <h1>儿科治疗执行</h1>
        <p>记录体重剂量、治疗方案、执行反应与监护人沟通，异常风险进入复核和随访。</p>
      </div>
      
    </div>

    <ClinicalPageState v-if="patientLeaseQuery.isPending.value || encounterLeaseQuery.isPending.value || itemsQuery.isPending.value" kind="loading" message="正在读取儿科治疗记录" />
