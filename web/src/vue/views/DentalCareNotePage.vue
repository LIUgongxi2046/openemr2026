...
import { createDentalCareNote, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listDentalCareNotes } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'dental-care', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('DENTAL_CARE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'dental-care', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('DENTAL_CARE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'dental-care', 'items'],
  queryFn: () => listDentalCareNotes(patientLease.value!),
...
async function createNote() {
  ...
  try {
    await createDentalCareNote(lease, {
  ...
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        ...
        <h1>口腔护理与术后观察</h1>
        <p>记录口腔术前准备、疼痛出血评估、术后观察与宣教，高风险信号进入复核和随访。</p>
      </div>
