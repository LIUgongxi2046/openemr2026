...
import { createPediatricEvidence, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listPediatricEvidences } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-evidence', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('PEDIATRICS_EVIDENCE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-evidence', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('PEDIATRICS_EVIDENCE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-evidence', 'items'],
  queryFn: () => listPediatricEvidences(patientLease.value!),
...
async function createNote() {
  ...
  try {
    await createPediatricEvidence(lease, {
  ...
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        ...
        <h1>儿科诊疗证据</h1>
        <p>记录生长发育、症状评估、检查结果与处置证据；异常信号驱动监护人沟通和复核。</p>
      </div>
      
    </div>

    <ClinicalPageState v-if="patientLeaseQuery.isPending.value || encounterLeaseQuery.isPending.value || itemsQuery.isPending.value" kind="loading" message="正在读取儿科诊疗证据" />
