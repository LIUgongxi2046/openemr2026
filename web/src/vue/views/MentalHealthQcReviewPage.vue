...
import type { MentalHealthQcReviewWire } from '../../generated/contracts';
import { createMentalHealthQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listMentalHealthQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'mental-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('MENTAL_HEALTH_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'mental-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('MENTAL_HEALTH_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'mental-qc', 'records'],
  queryFn: () => listMentalHealthQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'CRISIS_HANDOVER' as MentalHealthQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as MentalHealthQcReviewWire['review_conclusion'],
  defectDescription: '',
...
async function createRecord() {
  ...
  try {
    await createMentalHealthQcReview(lease, {
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'CRISIS_HANDOVER' ? '危机交接' : '危机随访' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="CRISIS_HANDOVER">危机交接</option><option value="CRISIS_FOLLOWUP">危机随访</option>
            </select></label>
