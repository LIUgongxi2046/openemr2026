...
import { createObstetricCareNote, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listObstetricCareNotes } from '../../api/specialty-layers';
...
const itemsQuery = useQuery({
  ...
  queryFn: () => listObstetricCareNotes(patientLease.value!),
...
async function createNote() {
  ...
  try {
    await createObstetricCareNote(lease, {
...
<template>
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        <p class="eyebrow">专科中心 / 妇产</p>
        <h1>妇产护理与产后观察</h1>
        <p>记录孕产期生命体征、出血与疼痛风险、母婴观察和护理措施，异常信号必须升级处置。</p>
      </div>
