...
const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'reproductive-evidence', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('REPRODUCTIVE_EVIDENCE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'reproductive-evidence', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('REPRODUCTIVE_EVIDENCE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'reproductive-evidence', 'items'],
  ...
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        ...
        <h1>生殖诊疗证据</h1>
        <p>记录周期监测、实验室结果、胚胎与妊娠结局证据；异常结果进入复核和随访。</p>
      </div>
      
    </div>

    <ClinicalPageState v-if="patientLeaseQuery.isPending.value || encounterLeaseQuery.isPending.value || itemsQuery.isPending.value" kind="loading" message="正在读取生殖诊疗证据" />
