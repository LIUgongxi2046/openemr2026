...
import { createTcmCareNote, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listTcmCareNotes } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'tcm-care', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('TCM_CARE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'tcm-care', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('TCM_CARE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'tcm-care', 'items'],
  queryFn: () => listTcmCareNotes(patientLease.value!),
...
<template>
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        <p class="eyebrow">专科中心 / 中医</p>
        <h1>中医护理与辨证施护</h1>
        <p>记录辨证施护、饮食起居、穴位与中医护理技术执行，关注疗效反应和安全风险。</p>
      </div>
