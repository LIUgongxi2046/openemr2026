...
<template>
  <section data-page-root class="content admin-content vue-native-page"><div class="page-head"><div class="page-title"><h1>用户、人员与账户管理</h1><p>人员基本档案、登录账号、岗位任期、业务角色和临床资质分开管理</p></div><div class="head-actions"><button class="btn" type="button" @click="importOpen = true">批量导入</button><button class="btn" type="button" @click="panel = panel === 'RULES' ? 'NONE' : 'RULES'">生命周期规则</button><button class="btn primary" type="button" @click="createOpen = true">新建人员/账户</button></div></div>
    ...
    <template v-else><section class="admin-metrics"><article><span>有效人员</span><strong>{{ activePeople }}</strong><small>人员基本档案</small></article><article><span>活跃账号</span><strong>{{ activeAccounts }}</strong><small>统一登录账号</small></article><article><span>有效角色</span><strong>{{ activeRoles }}</strong><small>当前有效期</small></article><article><span>在效资质</span><strong>{{ credentials }}</strong><small>按人员去重</small></article></section><p v-if="notice" class="admin-notice" role="status">{{ notice }}</p>
      ...
      <div class="grid admin-list-detail">
        ...
        <aside v-if="selectedPerson" class="card"><div class="card-head">{{ selectedPerson.primary.person_display_name }} · 账户处置 <span class="status" :class="selectedPerson.primary.account_status === 'ACTIVE' ? 'green' : 'red'">{{ selectedPerson.primary.account_status ? clinicalCodeLabel(selectedPerson.primary.account_status) : '未开户' }}</span></div><div class="card-body"><div class="folder-row">人员状态<span>{{ selectedPerson.primary.person_status === 'ACTIVE' ? '在职' : '离岗' }}</span></div><div class="folder-row">账户状态<span>{{ selectedPerson.primary.account_status ? clinicalCodeLabel(selectedPerson.primary.account_status) : '未开户' }}</span></div><div class="folder-row">有效角色<span>{{ roleSummary(selectedPerson.roles) }}</span></div><div class="folder-row">最后登录<span>{{ lastLogin(selectedPerson.primary) }}</span></div><div class="folder-row">历史署名<span>保留</span></div><div class="folder-row">角色有效期<span>{{ formatDate(activeRole(selectedPerson.roles).role_valid_until) }}</span></div><div class="notice hard"><div class="notice-title">停用不会删除历史署名</div>账户停用和角色结束均写入数据库；既有病历签名、审计证据与历史关系继续保留。</div><div class="admin-actions vertical"><button class="btn primary" type="button" @click="exportRevocationChecklist">生成撤权与任务转派清单</button><button class="btn" type="button" :disabled="selectedPerson.primary.account_status !== 'ACTIVE' || Boolean(busy)" @click="actionTarget = { item: selectedPerson.primary, action: 'account' }">停用账号</button><button class="btn" type="button" :disabled="activeRole(selectedPerson.roles).role_status !== 'ACTIVE' || Boolean(busy)" @click="actionTarget = { item: activeRole(selectedPerson.roles), action: 'role' }">结束当前角色</button></div></div></aside>
      </div>
    </template>
