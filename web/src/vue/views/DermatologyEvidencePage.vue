...
import { createDermatologyEvidence, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listDermatologyEvidences } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'dermatology-evidence', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('DERMATOLOGY_EVIDENCE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'dermatology-evidence', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('DERMATOLOGY_EVIDENCE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'dermatology-evidence', 'items'],
  queryFn: () => listDermatologyEvidences(patientLease.value!),
...
async function createNote() {
  ...
  try {
    await createDermatologyEvidence(lease, {
  ...
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        ...
        <h1>皮肤诊疗证据</h1>
        <p>记录皮损评估、影像、用药反应与处置证据；高风险信号驱动复核和随访。</p>
      </div>
      
    </div>

    <ClinicalPageState v-if="patientLeaseQuery.isPending.value || encounterLeaseQuery.isPending.value || itemsQuery.isPending.value" kind="loading" message="正在读取皮肤诊疗证据" />
