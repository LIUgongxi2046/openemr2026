...
import { createOphthalmologyEvidence, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listOphthalmologyEvidences } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-evidence', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('OPHTHALMOLOGY_EVIDENCE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-evidence', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('OPHTHALMOLOGY_EVIDENCE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-evidence', 'items'],
  queryFn: () => listOphthalmologyEvidences(patientLease.value!),
...
async function createNote() {
  ...
  try {
    await createOphthalmologyEvidence(lease, {
  ...
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        ...
        <h1>眼科诊疗证据</h1>
        <p>记录视力、眼压、影像与围术期评估证据；异常结果驱动复核与术后随访。</p>
      </div>
      
    </div>

    <ClinicalPageState v-if="patientLeaseQuery.isPending.value || encounterLeaseQuery.isPending.value || itemsQuery.isPending.value" kind="loading" message="正在读取眼科诊疗证据" />
