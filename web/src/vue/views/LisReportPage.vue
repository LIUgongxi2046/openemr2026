...
const activeReport = computed(() => labReports.value[0] ?? null);
...
<template>
  <section data-page-root class="content vue-native-page">
    ...
    <div v-else>
      ...
      <div v-else class="grid report-layout record-real-report"><section class="card"><div class="card-head">生化检验报告 <span class="status green">{{ activeReport.report_status === 'CORRECTED' ? '已更正' : '已审核' }} · v{{ activeReport.version_no }}</span></div><table class="table lab-table"><thead><tr><th>项目</th><th>结果</th><th>标志</th><th>单位</th><th>参考范围</th><th>方法</th></tr></thead><tbody><tr v-for="observation in activeReport.observations" :key="observation.observation_id" :class="{ 'result-alert': observation.abnormal_flag !== 'NORMAL' }"><td><b>{{ observation.item_name }}</b></td><td>{{ observation.numeric_value ?? observation.text_value }}</td><td>{{ abnormalLabel(observation.abnormal_flag) }}</td><td>{{ observation.unit }}</td><td>{{ observation.reference_low ?? '—' }}–{{ observation.reference_high ?? '—' }}</td><td>来源报告</td></tr></tbody></table><div class="card-body"><div class="trend-chart"><div v-for="(observation, index) in activeReport.observations.slice(0, 4)" :key="observation.observation_id" :class="{ active: index === 0 }"><b>{{ observation.numeric_value ?? observation.text_value }}</b><span>{{ observation.item_name }}</span></div></div><div v-if="criticalOpenCount" class="notice hard"><div class="notice-title">待接收危急值 {{ criticalOpenCount }} 项</div>查看不等于接收；请到结果工作台完成复读确认与临床处置。</div></div></section><aside class="card"><div class="card-head">申请—标本—报告证据</div><div class="card-body"><div class="folder-row">申请<span>…{{ activeReport.order_id.slice(-8) }}</span></div><div class="folder-row">报告时间<span>{{ formatDate(activeReport.reported_at) }}</span></div><div class="folder-row">连接器<span>{{ activeReport.source_system }}</span></div><div class="folder-row">报告版本<span>v{{ activeReport.version_no }}</span></div><div class="folder-row">业务对账<span>患者/就诊/医嘱一致</span></div><div class="folder-row">来源键<span>…{{ activeReport.source_report_key.slice(-8) }}</span></div><div class="notice rule"><div class="notice-title">临床处置待办</div>异常结果需要医生确认；报告更正只追加版本，不覆盖既往引用。</div><button class="btn primary record-reference-action" type="button" @click="selectedResult = activeReport">确认并作为来源</button></div></aside></div>
    </div>
