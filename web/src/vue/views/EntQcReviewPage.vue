...
import type { EntQcReviewWire } from '../../generated/contracts';
import { createEntQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listEntQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ent-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('ENT_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ent-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('ENT_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'ent-qc', 'records'],
  queryFn: () => listEntQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'AIRWAY_RISK_HANDOVER' as EntQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as EntQcReviewWire['review_conclusion'],
  defectDescription: '',
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'AIRWAY_RISK_HANDOVER' ? '气道风险交接' : '耳鼻喉记录' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="AIRWAY_RISK_HANDOVER">气道风险交接</option><option value="RECORD">耳鼻喉记录</option>
            </select></label>
