...
import { createEmergencyNursingNote, issueEmergencyLease, listEmergencyNursingNotes } from '../../api/emergency';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const leaseQuery = useQuery({
  queryKey: ['emergency', 'nursing', 'lease'],
  queryFn: () => issueEmergencyLease('EMERGENCY_NURSING'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const itemsQuery = useQuery({
  queryKey: ['emergency', 'nursing', 'items'],
  queryFn: () => listEmergencyNursingNotes(leaseQuery.data.value!),
...
async function createNote() {
  ...
  try {
    await createEmergencyNursingNote(lease, {
