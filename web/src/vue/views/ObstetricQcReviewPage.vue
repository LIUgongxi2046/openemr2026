...
import type { ObstetricQcReviewWire } from '../../generated/contracts';
import { createObstetricQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listObstetricQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'obgyn-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('OBGYN_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'obgyn-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('OBGYN_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'obgyn-qc', 'records'],
  queryFn: () => listObstetricQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'DELIVERY' as ObstetricQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as ObstetricQcReviewWire['review_conclusion'],
  defectDescription: '',
...
async function createRecord() {
  ...
  try {
    await createObstetricQcReview(lease, {
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'DELIVERY' ? '分娩记录' : '产前检查' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="DELIVERY">分娩记录</option><option value="ANTENATAL_EXAM">产前检查</option>
            </select></label>
