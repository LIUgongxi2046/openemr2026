...
<template>
  <main id="main-content" class="content emergency-access-page vue-native-page"><div class="page-heading admin-heading"><div><p class="eyebrow">临床工作域 / 安全越权</p><h1>紧急访问</h1><p>只用于患者危急且常规授权无法及时建立的场景。必须填写具体理由、选择短时限、明确确认风险，且不会绕过显式保护规则。</p></div><RouterLink class="button secondary" to="/outpatient">返回门诊</RouterLink></div>
