...
import type { DentalQcReviewWire } from '../../generated/contracts';
import { createDentalQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listDentalQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'dental-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('DENTAL_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'dental-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('DENTAL_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'dental-qc', 'records'],
  queryFn: () => listDentalQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'TREATMENT_RECORD' as DentalQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as DentalQcReviewWire['review_conclusion'],
  defectDescription: '',
...
async function createRecord() {
  ...
  try {
    await createDentalQcReview(lease, {
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'TREATMENT_RECORD' ? '治疗操作记录' : '口腔记录' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="TREATMENT_RECORD">治疗操作记录</option><option value="RECORD">口腔记录</option>
            </select></label>
