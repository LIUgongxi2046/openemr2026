...
import type { PediatricQcReviewWire } from '../../generated/contracts';
import { createPediatricQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listPediatricQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('PEDIATRIC_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('PEDIATRIC_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'pediatrics-qc', 'records'],
  queryFn: () => listPediatricQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'GROWTH_RECORD' as PediatricQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as PediatricQcReviewWire['review_conclusion'],
  defectDescription: '',
...
async function createRecord() {
  ...
  try {
    await createPediatricQcReview(lease, {
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'GROWTH_RECORD' ? '生长发育记录' : '随访记录' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="GROWTH_RECORD">生长发育记录</option><option value="FOLLOWUP_RECORD">随访记录</option>
            </select></label>
