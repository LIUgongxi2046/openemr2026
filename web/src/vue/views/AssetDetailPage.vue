...
import { issueMedicalRecordAssetLease, listMedicalRecordAssetIntegrityEvents, listMedicalRecordAssets, retireMedicalRecordAsset, updateMedicalRecordAsset, verifyMedicalRecordAssetIntegrity } from '../../api/records';
...
const busy = ref(false); const notice = ref(''); const editorOpen = ref(false); const retireOpen = ref(false); const retireReason = ref(''); const verifyOpen = ref(false); const observedHash = ref('');
...
async function verify(){const data=assetsQuery.data.value,asset=selected.value;if(!data||!asset||!/^[0-9a-fA-F]{64}$/.test(observedHash.value))return;busy.value=true;try{await verifyMedicalRecordAssetIntegrity(data.lease,asset,observedHash.value);notice.value='验真事件已写入不可变证据链。';verifyOpen.value=false;await assetsQuery.refetch();}catch(error){const next=toClinicalIssue(error);notice.value=`${next.code}：${next.message}`;}finally{busy.value=false;}}
...
<template>
  <section data-page-root class="content vue-native-page archive-content">
    ...
    <BusinessActionDialog :open="verifyOpen" title="新建资产验真" description="比较重新读取的哈希和编目锚点。" eyebrow="资产证据 / 验真" confirm-label="执行比对" :busy="busy" :confirm-disabled="!/^[0-9a-fA-F]{64}$/.test(observedHash)" @cancel="verifyOpen=false" @confirm="verify"><label>观测哈希<input v-model="observedHash" maxlength="64"/></label></BusinessActionDialog>
