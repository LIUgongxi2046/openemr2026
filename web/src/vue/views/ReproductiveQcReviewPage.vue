...
import type { ReproductiveQcReviewWire } from '../../generated/contracts';
import { createReproductiveQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listReproductiveQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'reproductive-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('REPRODUCTIVE_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'reproductive-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('REPRODUCTIVE_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'reproductive-qc', 'records'],
  queryFn: () => listReproductiveQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'ART_CYCLE' as ReproductiveQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as ReproductiveQcReviewWire['review_conclusion'],
  defectDescription: '',
...
async function createRecord() {
  ...
  try {
    await createReproductiveQcReview(lease, {
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'ART_CYCLE' ? 'ART周期' : '胚胎移植' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="ART_CYCLE">ART周期</option><option value="EMBRYO_TRANSFER">胚胎移植</option>
            </select></label>
