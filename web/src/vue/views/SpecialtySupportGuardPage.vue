...
<template>
  <section data-page-root class="vue-boundary-page specialty-guard-page"><p class="eyebrow">专科支持门禁 / {{ routeId }}</p><h1>{{ definition?.title }}</h1><ClinicalPageState v-if="supportQuery.isPending.value" kind="loading" message="正在读取院级专科支持声明与证据有效期" /><ClinicalPageState v-else-if="issue" kind="error" :code="issue.code" :message="issue.message" @retry="supportQuery.refetch()" />
