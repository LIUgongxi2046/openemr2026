...
import type { EmergencyPreadmissionWire, EmergencyTriageAssessmentWire } from '../../generated/contracts';
import {
...
const capacityPercent = computed(() => Math.min(100, Math.round((queue.value.length / 24) * 100)));
...
<template>
  <section data-page-root class="content admin-content vue-native-page emergency-crud-page">
    <div class="page-head"><div class="page-title"><h1>急诊预检分诊与分区</h1><p>快速身份、生命体征、分诊依据、绿色通道和动态改区</p></div><div class="head-actions"><button class="btn" type="button" @click="rulesOpen=true">分诊规则 v3.4</button><button class="btn" type="button" @click="openCreate">新增分诊评估</button><button class="btn primary" type="button" @click="openPreadmissionCreate">新增急诊患者</button></div></div>
    ...
    <template v-else>
      <div class="metric-grid emergency-metrics" aria-label="分诊统计">
        ...
        <div class="metric"><div class="name">Ⅰ/Ⅱ级</div><div class="value">{{ currentAssessment?.triage_level === 'LEVEL_1' ? 1 : 0 }} / {{ currentAssessment?.triage_level === 'LEVEL_2' ? 1 : 0 }}</div><div class="trend">绿色通道 {{ currentAssessment?.immediate_action_required ? 1 : 0 }}</div></div>
        <div class="metric"><div class="name">待核身份</div><div class="value">{{ unregisteredCount }}</div><div class="trend">先救治路径已开启</div></div>
        <div class="metric"><div class="name">区域容量</div><div class="value">{{ capacityPercent }}%</div><div class="trend warning-text">按 24 个急诊单元测算</div></div>
      </div>
      ...
      <div class="grid secondary-grid emergency-prototype-layout">
        ...
        <aside class="card scroll-card emergency-prototype-side"><div class="card-head">分诊决策与改区</div><div class="card-body">
          ...
          <div class="form-row"><div class="label">当前区域</div><div class="field">{{ currentAssessment?.immediate_action_required ? '抢救区 · 立即处置' : '诊室 / 候诊区' }}</div></div>
    ...
    <AdminActionDialog v-model:open="rulesOpen" title="急诊分诊规则 v3.4" description="规则用于辅助判级，临床人员必须结合生命体征和危险信号确认。" eyebrow="急诊 / 分诊规则"><div class="emergency-rule-dialog"><div v-for="rule in [['Ⅰ级','濒危，立即抢救'],['Ⅱ级','危重，10 分钟内处置'],['Ⅲ级','急症，动态复评'],['Ⅳ级','非急症，常规候诊']]" :key="rule[0]"><span class="status" :class="rule[0]==='Ⅰ级'?'red':rule[0]==='Ⅱ级'?'amber':'blue'">{{ rule[0] }}</span><b>{{ rule[1] }}</b></div></div></AdminActionDialog>
