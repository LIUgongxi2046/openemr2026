...
import { createEntTreatment, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listEntTreatments } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ent-treatment', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('ENT_TREATMENT'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ent-treatment', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('ENT_TREATMENT'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'ent-treatment', 'items'],
  queryFn: () => listEntTreatments(patientLease.value!),
...
async function createNote() {
  ...
  try {
    await createEntTreatment(lease, {
  ...
  <section data-page-root class="content admin-content vue-native-page">
    <div class="page-heading admin-heading">
      <div>
        ...
        <h1>耳鼻喉治疗执行</h1>
        <p>记录治疗前评估、操作与用药执行、疗效和并发症风险，形成可追溯治疗闭环。</p>
      </div>
      
    </div>

    <ClinicalPageState v-if="patientLeaseQuery.isPending.value || encounterLeaseQuery.isPending.value || itemsQuery.isPending.value" kind="loading" message="正在读取耳鼻喉治疗记录" />
