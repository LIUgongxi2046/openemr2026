...
const patientLabel = computed(() => selectedPatientId.value === clinicalContext.patientId ? developmentCopy.patientName : `合成患者 …${selectedPatientId.value.slice(-6)}`);
...
async function callPatient(entry: WaitingQueueEntryWire) {
  ...
  try {
    ...
    notice.value = `候诊 #${updated.sequence_no} 已叫号；状态 ${updated.status}，版本 v${updated.row_version}。`;
...
<template>
  <main id="main-content" class="content vue-native-page outpatient-workspace-page">
    ...
    <template v-else-if="snapshot">
      <section class="patient-strip" aria-label="当前患者上下文"><div class="patient-avatar">{{ selectedPatientId === clinicalContext.patientId ? developmentCopy.patientAvatar : '合' }}</div><div><strong>{{ patientLabel }}</strong><span>患者 …{{ selectedPatientId.slice(-8) }} · 就诊 …{{ selectedEncounterId.slice(-8) }}</span></div><dl><div><dt>候诊状态</dt><dd>{{ selectedQueueEntry?.status ?? '已进入诊疗上下文' }}</dd></div><div><dt>上下文租约</dt><dd>…{{ snapshot.lease.lease_id.slice(-8) }}</dd></div><div><dt>快照完整性</dt><dd :class="{ danger: partialTimeline }">{{ partialTimeline ? '部分数据' : '完整' }}</dd></div></dl></section>
      ...
      <div class="outpatient-dashboard">
        ...
        <aside class="outpatient-actions">
          <section class="admin-panel"><header><div><h2>AI 摘要边界</h2><p>建议层，不是临床事实</p></div></header><div class="ai-source-state"><span>当前状态</span><strong>尚未生成当次 AI 摘要</strong><p>页面只呈现服务端临床事实。需要时进入 AI 助手，以当前租约和数据水位生成带引用、有效期和人工决策的候选。</p><dl><div><dt>来源水位</dt><dd>{{ snapshot.timeline.data_watermark.slice(0, 12) }}…</dd></div><div><dt>失效条件</dt><dd>切换患者 / 新版本 / 租约过期</dd></div></dl><RouterLink class="button secondary full" to="/ai-assistant">生成带来源的候选摘要</RouterLink></div></section>
