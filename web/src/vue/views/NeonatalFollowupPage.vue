...
import type { NeonatalFollowupRecordWire } from '../../generated/contracts';
import { createNeonatalFollowupRecord, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listNeonatalFollowupRecords } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'neonatal-followup', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('NEONATAL_FOLLOWUP'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'neonatal-followup', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('NEONATAL_FOLLOWUP'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'neonatal-followup', 'records'],
  queryFn: () => listNeonatalFollowupRecords(patientLease.value!),
...
async function createRecord() {
  ...
  try {
    await createNeonatalFollowupRecord(lease, {
