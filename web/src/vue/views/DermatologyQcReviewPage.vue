...
import type { DermatologyQcReviewWire } from '../../generated/contracts';
import { createDermatologyQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listDermatologyQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'dermatology-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('DERMATOLOGY_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'dermatology-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('DERMATOLOGY_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'dermatology-qc', 'records'],
  queryFn: () => listDermatologyQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'BIOLOGIC_SCREENING' as DermatologyQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as DermatologyQcReviewWire['review_conclusion'],
  defectDescription: '',
...
async function createRecord() {
  ...
  try {
    await createDermatologyQcReview(lease, {
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'BIOLOGIC_SCREENING' ? '生物制剂筛查' : '生物制剂随访' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="BIOLOGIC_SCREENING">生物制剂筛查</option><option value="BIOLOGIC_FOLLOWUP">生物制剂随访</option>
            </select></label>
