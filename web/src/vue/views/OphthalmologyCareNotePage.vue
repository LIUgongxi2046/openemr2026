...
import { createOphthalmologyCareNote, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listOphthalmologyCareNotes } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-care', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('OPHTHALMOLOGY_CARE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);
const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-care', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('OPHTHALMOLOGY_CARE'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);
const itemsQuery = useQuery({
  queryKey: ['specialty-layers', 'ophthalmology-care', 'items'],
  queryFn: () => listOphthalmologyCareNotes(patientLease.value!),
...
async function createNote() {
  ...
  try {
    await createOphthalmologyCareNote(lease, {
