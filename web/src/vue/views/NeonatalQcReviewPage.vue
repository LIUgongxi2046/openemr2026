...
import type { NeonatalQcReviewWire } from '../../generated/contracts';
import { createNeonatalQcReview, issueSpecialtyEncounterLease, issueSpecialtyPatientLease, listNeonatalQcReviews } from '../../api/specialty-layers';
import ClinicalPageState from '../components/ClinicalPageState.vue';
import { toClinicalIssue } from '../clinical-error';

const patientLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'neonatal-qc', 'patient-lease'],
  queryFn: () => issueSpecialtyPatientLease('NEONATAL_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const patientLease = computed(() => patientLeaseQuery.data.value);

const encounterLeaseQuery = useQuery({
  queryKey: ['specialty-layers', 'neonatal-qc', 'encounter-lease'],
  queryFn: () => issueSpecialtyEncounterLease('NEONATAL_QC'),
  retry: false, staleTime: 5 * 60_000, gcTime: 0,
});
const encounterLease = computed(() => encounterLeaseQuery.data.value);

const recordsQuery = useQuery({
  queryKey: ['specialty-layers', 'neonatal-qc', 'records'],
  queryFn: () => listNeonatalQcReviews(patientLease.value!),
...
const form = reactive({
  reviewedRecordType: 'WRISTBAND_VERIFICATION' as NeonatalQcReviewWire['reviewed_record_type'],
  reviewedRecordId: '',
  reviewConclusion: 'PASS' as NeonatalQcReviewWire['review_conclusion'],
  defectDescription: '',
...
async function createRecord() {
  ...
  try {
    await createNeonatalQcReview(lease, {
            ...
            <table class="admin-table">
              ...
              <tbody>
                <tr v-for="record in records" :key="record.review_id">
                  <td>{{ record.reviewed_record_type === 'WRISTBAND_VERIFICATION' ? '腕带标本核对' : '筛查记录' }}</td>
        ...
        <section class="admin-panel admin-form-panel">
          ...
          <form class="admin-form" @submit.prevent="createRecord">
            <label><span>复核记录类型</span><select v-model="form.reviewedRecordType">
              <option value="WRISTBAND_VERIFICATION">腕带标本核对</option><option value="SCREENING_RECORD">筛查记录</option>
            </select></label>
