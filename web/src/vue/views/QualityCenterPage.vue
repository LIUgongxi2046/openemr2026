...
const modules = [
  ...
  { icon: '级', title: '评级取证', description: '功能、应用范围、质量和证据快照', to: '/quality-rating' },
...
<template>
  <section data-page-root class="content vue-native-page quality-center-page">
    ...
    <section class="hub-hero" aria-label="医疗质量中心治理摘要"><div><span>质量与安全</span><h2>把每个质量指标落回患者、文书、规则、缺陷和整改证据</h2><p>功能存在不等于有效应用；中心同时呈现覆盖范围、数据质量、问题工单、整改复核与评级取证快照。具体病历和病案证据使用明确跨域入口查看。</p></div><div class="hub-score"><b>{{ leaseQuery.isPending.value || ratingQuery.isPending.value ? '…' : mappedProjects }}</b><span>已建档范围，其中 {{ verifiedProjects }} 项已验证</span></div></section>
