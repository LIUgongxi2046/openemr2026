async page => {
  ...
  const result = await page.evaluate(async () => {
    const [csv, titles] = await Promise.all([
      fetch('/ui-delivery/route-design-map.csv').then(response => response.text()),
      fetch('/ui-delivery/route-titles.json').then(response => response.json())
    ]);
    ...
    return { routes: routes.length, specialtyChecked, failures, finalRoute: location.hash };
  });
