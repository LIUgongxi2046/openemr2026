...
`ui-delivery/tokens.json -> token compiler -> CSS custom properties + TypeScript const + Storybook docs`。构建过程禁止业务包出现未在 Token 登记的 Hex 颜色。
...
`ui-delivery/route-design-map.csv` 继续作为 194 页视觉/状态事实源，`prototype/traceability.csv` 作为 FR/AC/SCR 事实源。S007 必须生成 `route-contract.generated.json`，每行包含：
