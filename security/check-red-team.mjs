...
const requiredSurfaces = new Set(['document sections', 'patient search', 'document command', 'context lease', 'API body', 'clinical text', 'model output', 'worker tool', 'proposal decision', 'SSE', 'signed document', 'tenant foreign key']);
const ids = new Set();
...
console.log(JSON.stringify({ payloads: ids.size, surfaces: 12, synthetic: true, gate: 'PASS' }));
